...
- **Why:** the defect is precisely that the indicator returns the same answer in both states, so any
  ...
  container-stop machinery and simply has no Service Bus arm.
- **Test home:** the new suite, guarded on `SERVICE_BUS_CONNECTION_STRING` with the `ignore:` form
...
| `packages/messaging-plugin/test/integration/service-bus-outage-real.test.ts` (new) | the broker end to end, real emulator | The §0 2×2, all four cells, plus the discriminating assertion that the running and stopped answers DIFFER. Guarded with `ignore:` on `SERVICE_BUS_CONNECTION_STRING` (§3.3); container restarted between runs per R11.                                                                                                                                                                                                                                                                                                                                                                                                    |
