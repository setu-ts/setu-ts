...
- **Decision:** `common` gains
  ...
  | MongoDB   | none — MongoDB's snapshot isolation is not the portable `serializable` contract | all four           |
...
| `database-plugin/test/integration/real-mongo-adapter.test.ts` (extended)   | `adapters/mongo/…`               | Every requested portable isolation level refuses by name before opening a driver transaction.                                                                                                                                                                                |
