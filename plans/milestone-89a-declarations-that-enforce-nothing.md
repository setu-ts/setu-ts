...
| `respondWithError`                      | `common/src/http.ts` (M70f seam)                                      | Answers in the application's configured error format from a package that may not import `@setu-ts/exceptions` (§2.2).                                         |
...
| `MultiTenancyPluginOptions`             | `multi-tenancy-plugin/src/interfaces/index.ts:90-106`                 | `resolver` (required), per-resolver bags, `database?`, `dataStore?`. `database` defaults `'column-per-tenant'`.                                               |
...
| C4 | The M89b half of `ROADMAP.md` states X18-2's guards "take the same treatment at `403`". Checked against `guards/index.ts:59-84`, the 403 path is already correct — what is wrong is the **absent-capability** case, which is a server misconfiguration and not a policy refusal. | `403` is wrong for that case; it belongs in M89b's status decision, not here. **This plan does not touch it** and flags the ROADMAP line for M89b to correct with its own reasoning. | Named as an M89b deliverable in that plan's §2, not silently inherited here.                                                   |
...
- **Decision:** no behaviour change. Its JSDoc gains a sentence saying it contributes `security: []`
  to the OpenAPI document and does **not** exempt a route from a guard.
...
- **Why:** the documented contract is singular and every other misapplied input in this CLI is
  ...
  defect.
- **Test home:** `test/unit/add-command.test.ts` — two positionals exit 2 and the manifest is
...
| `packages/decorator-plugin/src/decorators/security.ts`             | JSDoc only — the `enforceRoles` caveat on `@Roles`/`@Permissions`, and `@Public`'s document-only effect (§3.7).           |
...
| `PUBLIC_API.md`                                                    | `enforceRoles`; the `@Roles`/`@Permissions` enforcement statement; the isolation-strategy correction; C4's flag for M89b. |
...
| `decorator-plugin/test/unit/barrel-exports.test.ts`                 | `src/index.ts`                                  | The published surface is unchanged (M56 defect class).                                                                                                                                                                                                                                                          |
...
| `multi-tenancy-plugin/test/unit/barrel-exports.test.ts`             | `src/index.ts`                                  | Unchanged surface.                                                                                                                                                                                                                                                                                              |
| `cli/test/unit/add-command.test.ts` (extend)                        | `commands/add.ts`                               | Two positionals → exit 2, manifest untouched; one positional unchanged; the message names the count.                                                                                                                                                                                                            |
...
- **X18-2 and X19-1** — masked-500 refusals; **M89b**, which also owns C4's ROADMAP correction.
