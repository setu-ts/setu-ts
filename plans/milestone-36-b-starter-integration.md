...
| Starter option shape                           | `feat/m36-starters:packages/starters/rest-starter/src/options.ts:22-64`, `.../microservice-starter/src/options.ts:16`, `.../full-stack-starter/src/options.ts:24` | One optional arm per plugin; `MicroserviceStarterOptions extends RestStarterOptions` and `FullStackStarterOptions extends MicroserviceStarterOptions`. An arm added to `RestStarterOptions` is inherited by all three tiers.                                                                                                                                       |
| Starter gating idiom                           | `feat/m36-starters:packages/starters/rest-starter/src/rest-app.ts:44-45`                                                                                          | `...(options.database ? [DatabasePlugin(options.database)] : [])` — the established spread-gate for an opt-in arm.                                                                                                                                                                                                                                                 |
...
- **Test home:** `test/unit/rest-app.test.ts` asserts `DiPlugin` is absent by default and present
  when the arm is supplied; `test/integration/rest-app-integration.test.ts` asserts a decorated
  service resolves through the container when the arm is on, and through the registry when it is
  off.
...
- **Test home:** `test/unit/rest-app.test.ts` — one case per sub-arm asserting the plugin set, plus
  a case asserting `realtime: {}` adds none.
...
- **Test home:** `test/integration/rest-app-integration.test.ts` asserts `start()` rejects with the
...
| `packages/starters/rest-starter/src/rest-app.ts`                 | Add the four spread-gates (§3.4, §3.5).                                                                           |
| `packages/starters/rest-starter/src/index.ts`                    | Export `RealtimeArm`.                                                                                             |
| `packages/starters/microservice-starter/src/microservice-app.ts` | Inherit the arms through the REST builder; no new gate logic.                                                     |
| `packages/starters/full-stack-starter/src/full-stack-app.ts`     | Inherit the arms through the microservice builder; no new gate logic.                                             |
| `packages/cli/src/templates/nest.ts`                             | The `nest` template: wirings, `unsupported: {}`, and the three emitted source files (§3.7).                       |
| `packages/cli/src/templates/registry.ts`                         | Register the `nest` template so `--template nest` and `--help` list it.                                           |
...
| `packages/starters/rest-starter/test/unit/rest-app.test.ts`                                | `rest-app.ts`, `options.ts`                  | Default set contains none of the four plugins; each sub-arm adds exactly its plugin; `realtime: {}` adds none; `di` present adds `DiPlugin`.                                                                                                                                                                                      |
| `packages/starters/rest-starter/test/integration/rest-app-integration.test.ts`             | `rest-app.ts`                                | An app with `realtime: { sse: {} }` starts and serves; `realtime: { backplane: { transport: 'messaging' } }` on the REST tier rejects with the factory's message (§3.6); a decorated service resolves through the container with `di` on.                                                                                         |
| `packages/starters/microservice-starter/test/unit/microservice-app.test.ts`                | `microservice-app.ts`                        | The inherited arms work through the microservice builder; the `'messaging'` backplane transport succeeds here because the tier supplies `MessagingPlugin`.                                                                                                                                                                        |
| `packages/starters/full-stack-starter/test/unit/full-stack-app.test.ts`                    | `full-stack-app.ts`                          | Inherited arms work; the full set with every arm supplied still has no duplicate `provides` (the M36 collision guard, extended by four tokens).                                                                                                                                                                                   |
| `packages/starters/full-stack-starter/test/integration/full-stack-app-integration.test.ts` | `full-stack-app.ts`                          | All arms on, one kernel, `inject()` returns `200` — the only check that catches a duplicate name or provider among the imperatively-registering plugins.                                                                                                                                                                          |
