...
- **In scope:** X11-1 (`sdk` fetch receiver) and X7-3 (`grpc-plugin` + `telemetry-plugin` `npm:`
  ...
  as before it. Doc corrections C1–C5, the C7 ROADMAP reassignment and the C8 ARCHITECTURE
...
| C8 | **`ARCHITECTURE.md:1580` describes a retired seam as current.** The `grpc-plugin` Rules row states "`setRpcHandler?` on `IHttpAdapter` is the interception seam", while `packages/common/src/runtime.ts:475-486` marks that method `@deprecated`, "no longer called by the kernel" and "a no-op in current adapters" — M70a moved dispatch into the kernel terminal handler. An M70a doc miss in merged `main`, found in the same table row C3 touches. | The code is right; the table is stale. Corrected **on this branch** rather than on a `fix/…` branch — a deliberate deviation from §15.2, taken by the maintainer at plan time on the M59 / M70h precedent (same file region, one line, and a separate branch for one stale row costs more than it protects).                     | `ARCHITECTURE.md` — the `grpc-plugin` **Rules** row names the kernel-pipeline dispatch as the mechanism and drops the `setRpcHandler?` claim; the **Responsibilities** cell's "RPC handler interception" is re-worded to match. No other row changes, and M70i remains free to rewrite the section. |
