...
> **Status:** Planning. Branch: `feat/m65-functional-default`. `main` is protected — all work (implementation +
...
functions for application services, and omit both `DecoratorPlugin` and `DiPlugin`. The
class-based position is selected by `--template class-based`; it installs both plugins and causes
subsequent generation to emit decorated controllers, injectable services, constructor injection,
and class-oriented modules. The selection persists in the generated manifest and `generate`
derives its mode from the installed package set, so later invocations cannot silently revert to a
different style.

- **In scope:** CLI template composition, one coherent generator-mode detector, functional
  `generate module`, mode-aware service and controller output, `@Ctx()` in generated decorated
  write handlers, regression coverage that boots both worlds, and documentation for the revised
  CLI behavior.
...
| --------- | ------------------ | ----------------------- |
| Template selection | `packages/cli/src/templates/choice.ts:51-72` | Both `new` and workspace-member generation obtain a `TemplateDefinition` and `TemplateFeatures` in one call site. |
| Current independent DI option | `packages/cli/src/templates/choice.ts:51-55`; `packages/cli/src/templates/di.ts:46-53` | `--di` currently creates a decorator-independent third state by appending only `DiPlugin`; this conflicts with M65's two-position design. |
| REST composition | `packages/cli/src/templates/rest.ts:42-65,100-116` | `REST_PLUGINS` currently includes `DecoratorPlugin`; `REST_SEAMS` and all generated config artifacts are derived from that package set. |
| Current class-template composition | `packages/cli/src/templates/nest.ts:125-152` | The current `nest` template is built from the REST set and appends `DI_WIRING`; M65 renames it to framework-neutral `class-based`, the one template that installs both opt-in plugins. |
...
| Domain aggregate output | `packages/cli/src/schematics/module.ts:40-203`; `packages/cli/src/utils/module-scanner.ts:49-103` | `module` is decorator-only today and its scanner admits a module only when controller and service files exist; functional modules therefore need a source-backed, mode-specific admission rule and aggregate barrel. |
...
- **Decision:** Treat the generated manifest's package imports as the persisted selector. A project
  containing both `decorator-plugin` and `di-plugin` is NestJS-shaped; a project containing neither
  is functional. The CLI rejects a partial pair for style-sensitive generation with a migration
  diagnostic, instead of producing an incoherent third style. `--template class-based` is the only stock
  selector that writes the pair. Remove the independent `--di` flag from new-project and
  workspace-member template choice.
- **Why:** `detectPlugins` already reads exactly the persisted data used for gating and never boots
  ...
  preserving decorator-only or DI-only generation would contradict M65's explicit single axis.
...
- **Decision:** Remove `DecoratorPlugin` from `REST_PLUGINS`, recompute REST seams from that
  ...
  no-template host functional. Rename the current `nest` template to `class-based`, then build its
  plugin list from the functional REST list plus exactly one
  decorator wiring and exactly one DI wiring; its explicit example classes and decorator seam
  inputs remain there.
...
- **Decision:** Add an internal generator-mode classifier fed by `SchematicOptions.plugins`; do not
  ...
  seam barrel. NestJS-shaped `service` emits the existing `@Injectable` class and barrel. The
  controller schematic remains available only in the NestJS-shaped mode and its generated create
  method accepts `@Ctx()` to return a true `201` response.
- **Why:** The existing published schematic contract already carries the source of truth. Widening it
  would expose a duplicate field that custom schematics could omit or contradict.
...
- **Decision:** Make `generate module` ungated and branch it by generator mode. Functional output is
  a module directory containing a plain data-access function module, a `register<Name>Routes`
  router module that closes over those functions, a focused function test, an index file, and a
  managed functional aggregate barrel. The functional aggregate barrel exports one registration
  function which the existing route setup seam calls. NestJS-shaped output preserves the existing
  controller/service classes, `@Inject` constructor seam, and controller/service aggregate arrays.
  The module scanner and barrel renderer will admit and render only the shape corresponding to the
  target's detected mode.
...
- **Decision:** Extend the existing real-project test harness to scaffold a functional REST project
  and a `nest` project, generate the relevant service, route or module artifacts, repoint imports at
  this workspace, type-check them, boot each application, and request its generated endpoints.
  Assert the functional module's `POST` response is `201`; assert the decorated controller's
  `@Ctx()` path also produces `201` and a `Location` header when the generated output specifies one.
...
| ------ | -------- | ----------------------------- |
| Generated manifest imports for `decorator-plugin` and `di-plugin` | `detectPlugins()` in `generate`, then the internal mode classifier | Both imports select NestJS-shaped output; neither selects functional output; exactly one is refused for style-sensitive schematics. |
...
| File | Purpose |
| ---- | ------- |
| `src/templates/choice.ts` | Remove independent DI selection and describe `nest` as the single class-style selection. |
| `src/templates/di.ts` | Retain one DI wiring constant for nest composition; remove the feature-appending function that creates a third state. |
...
| `src/templates/class-based.ts` | Framework-neutral replacement for `nest.ts`; adds the decorator and DI pair to functional REST, retains showcase files, and carries class-mode seams. |
| `src/utils/generator-mode.ts` | Internal, pure classifier for functional, NestJS-shaped, and invalid partial package sets. |
| `src/commands/generate.ts` | Classify the detected package set before style-sensitive generation and present the migration diagnostic for a partial pair. |
| `src/schematics/registry.ts` | Remove the decorator-only module gate and centralize which schematic is style-sensitive. |
| `src/schematics/service.ts` | Emit exported functions in functional mode and `@Injectable` classes only in NestJS-shaped mode. |
| `src/schematics/controller.ts` | Emit only in class mode and use the committed `@Ctx()` decorator for status-sensitive handlers. |
| `src/schematics/module.ts` | Render functional route-and-function aggregates or the existing decorated aggregate according to the classifier. |
| `src/schematics/module-barrel.ts` | Render the matching functional registration aggregate while preserving class-mode exports. |
| `src/utils/module-scanner.ts` | Admit and list functional or class-mode module shapes without creating an invalid aggregate import. |
| `src/templates/module-seam.ts` | Route functional module aggregate registration through the existing generated configuration seam. |
...
| `test/unit/generator-mode.test.ts` | `utils/generator-mode.ts` | Neither package gives functional mode, both give NestJS-shaped mode, and each partial pair returns the named migration diagnostic. |
| `test/unit/generate-command.test.ts` | `commands/generate.ts`, `schematics/registry.ts` | Detected imports are passed through the classifier; functional module is no longer plugin-gated; partial configuration writes no files. |
...
| `test/unit/schematics/module.test.ts` | `schematics/module.ts`, `schematics/module-barrel.ts` | Functional aggregate contains a function data seam and route-registration function; class mode retains constructor injection; both output sets match their stated `GeneratedFile[]` paths. |
| `test/unit/module-seam.test.ts` | `templates/module-seam.ts` | Functional module aggregate is imported and called from generated configuration while class mode still supplies decorator controller and service arrays. |
| `test/unit/utils/module-scanner.test.ts` | `utils/module-scanner.ts` | Scanner admits only complete functional shape in functional mode and only complete class shape in class mode; incomplete directories are skipped. |
| `test/unit/generate-module-command.test.ts` | `commands/generate.ts`, module scanner, module schematic | Existing functional modules regenerate a stable aggregate and conflict checks remain before writes. |
...
| `test/e2e/seam-probe.test.ts` | template seams and generated module wiring | Functional module route and NestJS-shaped module controller both answer requests; write responses demonstrate real `201` behavior. |
...
2. Remove `DiPlugin` from `class-based`; class-mode generation must refuse the partial package pair.
3. Replace the functional module setup call with no registration; the booted functional-module probe must return `404`.
4. Remove `@Ctx()` from the decorated write handler; the class-mode behavioral test must fail to demonstrate the required `201` response.

## 8. Risks & mitigations

- Existing projects containing exactly one of the two opt-in packages cannot be described by the new
  axis → style-sensitive generation refuses before writing and names the explicit migration to
  functional or NestJS-shaped composition.
...
- Any common, kernel, decorator-plugin, or DI-plugin API change belongs to its owning milestone;
  M65 consumes their committed public surfaces only.
