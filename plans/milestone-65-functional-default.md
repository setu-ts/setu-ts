...
- **Decision:** Make `generate module` ungated and branch it by generator mode. Functional output is
  a module directory containing a plain data-access function module, a `register<Name>Routes` router
  module that closes over those functions, a focused function test, an index file, and a managed
  functional aggregate barrel. The functional aggregate barrel exports one registration function
  which the existing route setup seam calls. Class-based output preserves the existing
...
- **Decision:** Extend the existing real-project test harness to scaffold a functional REST project
  ...
  controller's `@Ctx()` path also produces `201` and a `Location` header when the generated output
  specifies one.
