...
> **Status:** Planning. Branch: `feat/m36c-react-router-skeleton`. `main` is protected — all work
...
- **In scope:**
  1. A `full-stack` CLI template emitting a React Router 7 framework-mode skeleton: the
...
- **Decision:** one new optional field, `appFactory?: Wiring`. When present, `configModule` emits
  `const app = <symbol>(<args ?? ''>);` and imports `<symbol>` from `@hono-enterprise/<pkg>`,
  instead of `createApplication({ plugins: [...] })`; `plugins` is then required to be empty and
  `middleware` still applies. When absent, rendering is byte-identical to today.
...
- **Test home:** `packages/config-plugin/test/unit/config-plugin.test.ts` asserts an injected
...
No new symbol is added to `packages/common`, and no new capability token is invented — every
capability the skeleton consumes (`SSE`, `SECRETS`, `TELEMETRY`, `LOGGER`, `SESSION`, `SSR`) was
committed by an earlier milestone. `sessionContext` is emitted **into the scaffolded project**, not
exported from any package, so it is app code and carries no public-API obligation.
...
| `packages/config-plugin/test/unit/config-plugin.test.ts` (existing)                | `plugin/config-plugin.ts`                                                         | **Both entry points under a non-default config produce identical output** (§3.8); an injected `instance` is registered verbatim with **no** env read (runtime fake whose `env` throws); absent `instance` keeps today's behaviour.                                                 |
...
- **The emitted `.tsx` and Vite files cannot be `deno check`ed**, so the drift gate proves less for
  ...
  coverage.
- **The milestone spans three packages** (`config-plugin`, `full-stack-starter`, `cli`), against
  CLAUDE.md's one-package-per-milestone rule — the same risk M36b carried. Mitigation: §3.7–§3.9
  (config) are written as a self-contained design with their own contract rows and test files, and
  are **separable**: they can ship as a first PR before the skeleton if review grows unwieldy.
  Recommended if the diff exceeds ~1500 lines.
