...
| `StoredJob` — no transport handle field                                  | `packages/queue-plugin/src/interfaces/index.ts:95`                                                                                                                                                      | `{ id, name, data, attempts, maxAttempts, availableAtMs }` as committed. AS BUILT this milestone adds an optional `claimToken?` and widens the internal `QueueAdapter` settle methods to carry it, because keying the adapter-side handle map by `id` alone cannot distinguish a settle for the current delivery from one for a superseded delivery — see the DEVIATION bullet in §3.4. Neither `StoredJob` nor `QueueAdapter` is barrel-exported, so no published surface changes.                                                                                                                                                                                                                                  |
...
- **Test home:** `test/unit/pubsub-broker.test.ts` (recording `IPubSubTransport` fake: publish →
  ...
  emitter-to-promise bridge, and `ack`/`nack` routing) + the real-module load+adapt case AS BUILT
  inside `test/unit/pubsub-broker.test.ts` (§6).
...
- **Test home:** `test/unit/service-bus-broker.test.ts` (recording `IServiceBusTransport` fake:
  ...
  `processMessage`/`processError` are wired) + the real-module load+adapt case AS BUILT inside
  `test/unit/service-bus-broker.test.ts` (§6).
...
- **DEVIATION AS BUILT — the claim token was promoted onto the seam.** The map above is keyed by
  `id` alone, which cannot tell a settle for the CURRENT delivery from one for a superseded delivery
  of the same job: after a requeue the message returns with a NEW `ReceiptHandle`, and a late settle
  from the previous attempt would then target it. The implementation therefore added an opaque
  `claimToken` (`<id>:<receiptHandle>:<reservedAtMs>`) that `reserve` both stores and returns on the
  `StoredJob`, and widened the INTERNAL `QueueAdapter` seam so `ack`/`requeue`/`deadLetter` take it
  and can refuse a stale one. `StoredJob` gains an optional `claimToken?`, `JobRunnerAdapter` in
  `job-processor.ts` matches, and `MemoryQueue`/`RedisQueue`/`RabbitMqQueue` accept and ignore the
  argument (they have no transport-level claim, so `runJob` falls back to the job id). Nothing here
  is barrel-exported, so no published surface changes. Recorded after the fact: it shipped with no
  plan entry, which is how the wiring defect below survived — the seam grew a parameter whose only
  correct value came from `reserve`, while the caller kept passing the job id, so EVERY SQS settle
  failed its own claim check and no message was ever deleted, requeued, or dead-lettered. Fixed in
  `job-processor.ts` (pass `storedJob.claimToken ?? storedJob.id`) with the integration coverage the
  next bullet describes.
- **The SQS arm is proven through the kernel, not only through the adapter.** Every adapter-level
  test supplies the claim token itself, which is exactly the value the real caller produces, so the
  adapter can be entirely correct while the path an application takes is broken. The mandated
  integration coverage lives in `test/integration/sqs-arm-integration.test.ts` over the
  contract-honouring `test/fixtures/fake-sqs-transport.ts` (a delivered message goes invisible for
  its visibility window and RETURNS when it lapses, so a missing settle shows up as redelivery
  rather than as nothing at all). Three of its four cases fail without the `claimToken` fix.
...
| `adaptPubSubModule`, `loadPubSubModule`                                              | functions     | `GcpPubSubBroker.connect()` lazy path; `pubsub-adapter.test.ts` (pure, fake SDK module); the real-module load+adapt case inside `pubsub-broker.test.ts`. Mirrors `adaptSesModule`/`loadSesModule`. |
| `adaptServiceBusModule`, `loadServiceBusModule`                                      | functions     | `ServiceBusBroker.connect()`; `service-bus-adapter.test.ts`; the real-module case inside `service-bus-broker.test.ts`.                                                                             |
...
| `adaptSqsModule`, `loadSqsModule`, `adaptSnsModule`, `loadSnsModule`                 | functions     | `SqsQueue`/`SnsPublisher` lazy paths; their `*-adapter.test.ts`; the real-module cases inside `sqs-queue.test.ts` / `sns-publisher.test.ts`.                                                       |
...
| real-module load+adapt cases, AS BUILT inside each backend's own test file   | each backend's lazy path                                          | No separate `*-real-import.test.ts` files were created; the cases live in `pubsub-broker.test.ts`, `service-bus-broker.test.ts`, `sqs-queue.test.ts`, and `sns-publisher.test.ts`. Each loads the REAL `npm:` module (pinned in `deno.lock`, so it resolves rather than skipping), adapts it, and asserts every port member is a function — the assertion that catches SDK drift. Service Bus uses a WELL-FORMED dummy connection string, because `new ServiceBusClient` throws on a malformed one.                                                                                 |
...
| `queue-plugin/test/e2e/sqs-elasticmq.test.ts`                                | `sqs-queue.ts` against a real server                              | §3.9: real queues + a DLQ against ElasticMQ. AS BUILT this drives `SqsQueue` DIRECTLY rather than a kernel app — it proves the adapter's SQL-level behaviour (enqueue→reserve→ack, per-name isolation, visibility backoff, DLQ landing) against a real server, and deliberately NOT the `QueueService` wiring, which `test/integration/sqs-arm-integration.test.ts` owns. Stated because the distinction is load-bearing: an adapter-level e2e passes green over a broken settle path, which is what happened.                                                                      |
| `queue-plugin/test/integration/sqs-arm-integration.test.ts` (new)            | `queue-plugin.ts` + `sqs-queue.ts` end-to-end                     | The `'sqs'` arm through a real `createApplication` over `fake-sqs-transport.ts`: success deletes, failure changes visibility, exhaustion lands on the DLQ, and two job names do not consume each other. AS BUILT this is its own file rather than an extension of `queue-integration.test.ts`, whose harness is a hand-rolled fake context and never boots a kernel. Three of the four cases fail without the `claimToken` fix.                                                                                                                                                     |
