...
- **Test home:** `packages/queue-plugin/test/e2e/sqs-elasticmq.test.ts` — guarded on
  ...
  `defaultMaxAttempts`, and that `attempts` observed by the processor climbs 1→2→3. Plus a
  `test/apps-gate.test.ts`-style assertion in `packages/queue-plugin/test/unit/` pinning the CI
  service, the port mapping, the env var, and the scoped grant, so dropping the wiring turns the
  suite red instead of silently skipping (the M53 code-review lesson).
...
| `queue-plugin/test/unit/ci-wiring.test.ts` (new)                             | none (gate wiring)                                                | Pins the ElasticMQ service, the port mapping, `SQS_ENDPOINT_URL`, and the scoped `net` grant in the manifest, so removing the CI wiring fails the suite instead of silently skipping the e2e (§3.9, the M53 code-review lesson). Both assertions verified to fail when the wiring is broken.                                                                                                                                                                                                                                                                                        |
...
- **A guarded test that silently never runs** → `ci-wiring.test.ts` pins the CI service, port, env
  var, and grant (§3.9); the suite is run under both conditions per §7.
