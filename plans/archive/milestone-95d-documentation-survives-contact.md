...
| ------------------------------------------------------ | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `test/unit/check-since-tags.test.ts` (new)             | `scripts/check-since-tags.ts`        | **A tag naming a version whose tarball HAS the file but not the exported symbol FAILS** — the case that distinguishes §3.4's symbol-level gate from the file-level one it replaced, and without which a file-level implementation passes this table (added after review, PR #309, CodeRabbit); a tag naming a version whose tarball lacks the file entirely FAILS too, as the degenerate case; a tag naming one that has both passes; a version ahead of the registry is SKIPPED, not failed; **an injected network failure exits `0`** with the reason on stderr and `verified: 0` on stdout — asserted as the literal exit code, since exit 77 inside `check:docs`' `&&` chain is what finding 5 reports. |
...
1. Revert the six `@since` corrections → `check:since-tags` fails naming `packages/common/src/form/`
   and the version. This is the control that proves the gate would have caught D12/D13. 1b. Reduce
   the gate to a file-level check → the symbol-absent case above passes, which is the finding-4
   defect reproduced: a symbol added to a long-lived file keeps any `@since` at all.
