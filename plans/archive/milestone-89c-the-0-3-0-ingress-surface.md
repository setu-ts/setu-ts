...
§3.1's promise change adds **no** symbol; §3.1b adds `InMemoryBrokerOptions` (row above).
`barrel-exports.test.ts` in `common`, `messaging-plugin` and `multi-tenancy-plugin` pins that
nothing else moved (the M56 defect class).
