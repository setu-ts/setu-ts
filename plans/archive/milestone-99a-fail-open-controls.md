...
| The defaults have TWO application points   | `packages/logger-plugin/src/plugin/logger-plugin.ts:110-114,250`; `packages/logger-plugin/src/loggers/console-logger.ts:92-97`; `packages/logger-plugin/src/loggers/pino-logger.ts:43,74-77` | (a) `composeLoggerRedaction` builds ONE service from policy + legacy paths and passes it as `redaction` to BOTH transports; (b) `ConsoleLogger` ALSO compiles the raw default `redact` array. Pino receives a raw list only when the caller explicitly supplies `redact`. |
...
- **Why this covers pino:** the composed service from `composeLoggerRedaction` is handed to BOTH
  transports as `redaction`, and on pino it runs "after metadata normalization and before Pino"
  (`packages/logger-plugin/src/loggers/pino-logger.ts:76-77`). The default plugin path deliberately
  omits Pino's raw `redact` option; a caller-supplied list still reaches Pino unchanged. The
  composed service is therefore the one default-path redaction mechanism both transports share.
- **Test home:** `test/unit/redaction-defaults-casing.test.ts` boots a kernel application with stock
  `LoggerPlugin()` for console and injected-Pino transports, because the exercise measured the leak
  identically on both.
...
- **Test home:** `test/unit/service-bus-reachability.test.ts`.
...
- **Test home:** `test/unit/service-bus-reachability.test.ts` asserts the refusal is unchanged.
...
| `packages/logger-plugin/test/unit/redaction-defaults-casing.test.ts`         | `loggers/console-logger.ts`, `loggers/pino-logger.ts`, `plugin/logger-plugin.ts` | with stock `LoggerPlugin()`, a record built from `Object.fromEntries(new Headers({Authorization, Cookie}).entries())` has BOTH redacted through console and injected-Pino transports. The default Pino path omits the raw list and relies on the composed service; a caller-supplied `redact: ['X-Tok']` still misses `x-tok` |
