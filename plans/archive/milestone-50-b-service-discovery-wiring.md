...
| C2 | The `microservice` template's `unsupported` reason names only "messaging and queue" as the Workers blocker; `service-discovery-plugin` is now also in the set and its DNS-SRV arm needs `runtime.dns`, absent on Workers. | **Leave the reason unchanged.** The premise was wrong — the template wires the `'static'` arm, which contacts no backend, and only `DnsProvider` reads `runtime.dns`. Widening the reason (which this plan originally specified, and which was implemented and then reverted in code review) would state a blocker the generated config never meets and imply the plugin is unusable on Workers, when its static, Consul and Kubernetes arms are plain HTTP. | Add a JSDoc note recording why discovery is NOT part of the refusal, plus a test pinning that the reason cites no DNS/discovery blocker. |
