...
### 3.9 Every write into an existing `deno.json` is a merge, and a conflict refuses

- **Decision:** `setu devtool enable` never rewrites a manifest. It parses, sets the one key, and
  re-serializes at two-space indent — the `withDependency` precedent (`commands/add.ts:145`), which
  is already exported and whose own JSDoc records why it rewrites from the parsed object rather than
  by text surgery. Every other top-level key and every other task survives untouched. This is a
  decision `devtool enable` needs and the two creating entry points do not: `new --devtool` and
  `generate app --devtool` write the manifest themselves, so they have nothing to merge into.
- **Do NOT sort the `tasks` map, although the precedent sorts.** `withDependency` sorts because the
  dependency emitters it has to agree with sort, and an appended key would make a later regeneration
  reorder the file. `denoTasks` (`project-files.ts:847`) documents its block as "in a fixed order"
  and returns `{ start, test, ...extraTasks }` in insertion order, so applying the precedent
  verbatim would reorder a file the CLI itself wrote and put the merge permanently at odds with the
  emitter. This is the one place copying it would be wrong, named so a reviewer meets it as a
  decision rather than as an inconsistency.
- **Three outcomes per key, and the middle one is what makes the command idempotent.** A key that is
  absent is added. A key already present and byte-identical to what the planner would write is a
  no-op — the `undefined` return `withDependency` already uses for exactly this — so running
  `devtool enable` twice is not an error. A key present with a DIFFERENT value is refused by name,
  quoting the task and its current value, because a task a developer wrote is theirs and silently
  replacing it is the failure mode this whole letter is about.
- **Three writes, not one.** The set is larger than the `check` task alone: a project's `dev` task
  (§3.7), a project's `check` task (§3.1), and the **root workspace** `deno.json`'s `dev` task,
  whose grant gains the scoped `--allow-env` §3.4 needs. The third is the hard one and nothing else
  in this letter reaches it — it is a MODIFICATION of an existing string rather than an added key,
  and `managedFiles` (`commands/workspace.ts:49`) contains no `deno.json` at all, neither the root's
  nor a member's, so `setu workspace ports --reallocate` will never perform it and `devtool enable`
  is the only place it can happen. The string it widens is the CLI's own constant
  (`runtime-profile.ts:142`), so the same three outcomes apply: unmodified → widened, already
  carrying the three names → no-op, anything else → refused by name.
- **Only Deno is reachable here.** The other two workspace profiles carry a `package.json` root
  (`runtime-profile.ts:150`, `:165`) and the devtool is refused on both before any write (§3.2), so
  this merge has one manifest kind and needs no npm arm.
- **Test home:** `packages/cli/test/unit/devtool-manifest-merge.test.ts`, the conflict refusals in
  `test/unit/devtool-refusals.test.ts`, and the existing-project case in
  `test/e2e/devtool-e2e.test.ts`.
...
| File                                                                                            | Purpose                                                                                                                        |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `packages/cli/src/commands/devtool.ts`                                                          | The `setu devtool enable` command, its refusals, the pre-M98c factory detection (§3.1), and the `deno.json` task merge (§3.9). |
...
| `unit/devtool-manifest-merge.test.ts` | `commands/devtool.ts`                                       | Merging into an existing `deno.json` preserves every unrelated top-level key and every unrelated task, including a developer's own custom one; the `tasks` map keeps its emitted order rather than being sorted, which fails if the `withDependency` precedent is copied verbatim; a second `devtool enable` is a no-op rather than an error; and the root manifest's `dev` grant is widened in place with its `--allow-net` byte-identical to today.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `devtool enable` discards a developer's own tasks or configuration while adding its own: every
  write into an existing manifest is a merge from the parsed object (§3.9, the `add.ts:145`
  precedent), a differing task refuses by name rather than being replaced, and the e2e asserts an
  unrelated custom task and an unrelated top-level key survive.
