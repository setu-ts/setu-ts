...
> **Status:** Planning. Branch: `feat/m17-http-security-plugin`. `main` is protected — all work
...
security** — each registered as a global middleware function on the kernel pipeline. The plugin owns
no service and no capability token: like the `rateLimitMiddleware` shipped in M16b, it is a pure
middleware provider (`ctx.middleware.add(...)` in `register()`), and each middleware is also
exported as a standalone factory for per-route use. The package depends only on
...
- **In scope:** `HttpSecurityPlugin(options?)` factory; five middleware factories (`corsMiddleware`,
  `securityHeadersMiddleware`, `csrfMiddleware`, `requestSizeMiddleware`, `ipSecurityMiddleware`);
  the aggregate + per-concern option types; a barrel `src/index.ts`; a unit test file per `src/`
  file; an integration test exercising real `app.inject()` requests and short-circuit behavior;
  `PUBLIC_API.md` + `ARCHITECTURE.md` corrections; README.
- **NOT this milestone:** identity-layer security (`@hono-enterprise/auth-plugin`, M16/M16b:
  JWT/API-key/RBAC/guards/refresh/rate-limit); secret management (M23, `secrets-plugin`); IP
  allow/deny lists and per-route IP policy (deferred — see §9); a cookie- or token-store-based CSRF
  synchronizer pattern (deferred — `IRequest` has no cookie access; see §9); body parsing /
...
- **Decision:** `HttpSecurityPlugin` returns an `IPlugin` whose `register(ctx)` calls
  `ctx.middleware.add(concern, { priority, name })` for each enabled concern. It sets `provides: []`
  (omitted) and registers **no service** under any token. The five concerns are also exported as
  standalone `MiddlewareFunction` factories for per-route use.
...
- **Decision:** When `headers` is omitted the security-headers middleware still registers with a
  ...
  empty allowlist (deny all cross-origin) per §13.4. **The `enabled` check lives in exactly ONE
  place — each middleware factory reads its own `enabled` flag and, when `false`, returns a
  pass-through middleware (`(ctx, next) => next()`).** The plugin's `register()` forwards each
  present option block to its factory unchanged (plus the default-on rule for `headers`); it never
  duplicates the `enabled` check, so the plugin path and the standalone per-route path funnel
  through the same implementation (CLAUDE "one capability, one implementation").
- **Why:** Resolves conflict C1. Security headers are unconditionally safe and mandated on by §13.4;
  the other four need app context (CORS allowlist, CSRF trusted origins, a size budget, proxy-trust
  decision) to be meaningful, so opt-in avoids shipping a default that would be a no-op at best and
  a footgun at worst.
- **Test home:** `test/unit/http-security-plugin.test.ts` covers (a) default options → headers
  present, CORS/CSRF/size/IP inactive; (b) each opt-in block present → its concern active; (c)
  `enabled: false` on a present block → concern inactive; (d) one concern driven through BOTH entry
  points (plugin block and standalone factory) under the same non-default options, asserting
  identical observable behavior.

### 3.3 CSRF — stateless Origin/Referer validation (no cookies)

- **Decision:** For unsafe methods (`POST`, `PUT`, `PATCH`, `DELETE`), the CSRF middleware derives
  the source origin from the `Origin` header (falling back to `Referer`'s origin when `Origin` is
  absent) and compares its scheme+host against the trusted set: **the request's own origin
  (scheme+host derived from `request.url`) is ALWAYS implicitly trusted**, and the `trustedOrigins`
  allowlist (default `[]`) adds further allowed origins. A present source origin matching neither →
  short-circuit with **403** + JSON body. When BOTH headers are absent the request passes through
  regardless of the allowlist — non-browser clients (curl, server-to-server) send neither header,
  and blocking them would break every API client; the residual risk is documented in §8 and
  mitigated by `customHeader`. Safe methods (`GET`, `HEAD`, `OPTIONS`) always pass. An optional
  `customHeader` option additionally requires that header to be present on unsafe methods (a CSRF
  defense, since simple-form submits cannot set custom headers without a preflight). No cookie
  synchronizer token, no server-side token store.
- **Why:** `IRequest` has no cookie access (§1, conflict C3); the Origin/Referer check is the
  OWASP-recommended stateless CSRF defense and needs nothing beyond `request.headers` +
  `request.url`. Implicit self-origin trust is what makes ROADMAP's bare `csrf: { enabled: true }`
  safe to enable: browsers send `Origin` on every POST, including same-origin ones, so without it an
  empty allowlist would 403 every legitimate browser POST. The custom header adds defense-in-depth
  for API-style clients.
- **Test home:** `test/unit/csrf-middleware.test.ts` asserts: unsafe + same-origin `Origin` with NO
  `trustedOrigins` configured → pass (implicit self-trust); unsafe + origin in `trustedOrigins` →
  pass; unsafe + cross-origin not in the trusted set → 403 short-circuit (handler not run); unsafe
  - `Origin` absent but `Referer` present → `Referer`'s origin is used; unsafe + BOTH headers absent
    (allowlist empty AND non-empty) → pass-through; safe method → pass-through; `customHeader` set +
    header absent → 403; `customHeader` set + header present → pass-through.
...
- **Decision:** The request-size middleware reads the request's `Content-Length` header; if it is
  present and exceeds `maxBodySize` (bytes, **default `1_048_576` — 1 MiB, matching ROADMAP's
  example value**), it short-circuits with **413 Payload Too Large** + JSON body without calling
  `next()` or reading the body. Requests with no `Content-Length` (or a value within the limit) pass
  through. A `Content-Length` that does not parse as a non-negative integer is treated as absent
  (pass through) — rejecting malformed framing is the HTTP server's job, not this middleware's.
...
- **Test home:** `test/unit/request-size-middleware.test.ts` asserts: `Content-Length` over limit →
  ...
  pass-through; non-numeric/negative `Content-Length` → pass-through; the 1 MiB default applies when
  `maxBodySize` is omitted; response body shape on 413.
...
- **Test home:** `test/unit/ip-security-middleware.test.ts` asserts: `trustProxy` +
  `X-Forwarded-For:
  1.2.3.4, 10.0.0.1` → `state.get('clientIp') === '1.2.3.4'`;
  `trustProxy: false` → falls back to `request.ip`; missing header → `request.ip` fallback; `next()`
  always called.
...
- **Decision:** `origin` accepts `boolean` (`true` = reflect the request Origin, `false` = deny
  all), a single `string`, a `readonly string[]` allowlist, or
  `(origin, ctx) => string | boolean |
  Promise<…>`. On a match the middleware sets
  `Access-Control-Allow-Origin` (the specific origin, never `*` when `credentials: true`), appends
  the request Origin to `Vary`, and sets `Access-Control-Allow-Credentials: true` when
  `credentials: true`. An `OPTIONS` request carrying `Origin` + `Access-Control-Request-Method` is
  treated as a **preflight**: on an allowed origin it responds **204** with
  `Access-Control-Allow-Methods`/`Access-Control-Allow-Headers`/`Access-Control-Max-Age` (from
  options) and **short-circuits (no `next()`)**; on a disallowed origin it responds 204 with no CORS
  headers (also short-circuiting). Non-preflight requests with an allowed Origin get CORS headers
  added then call `next()`; non-preflight requests with a DISALLOWED Origin call `next()` with no
  CORS headers added (the browser enforces the block — the server does not reject); requests with no
...
- **Test home:** `test/unit/cors-middleware.test.ts` covers each `origin` variant, credentials
  reflection, preflight 204 short-circuit (handler not run), disallowed-origin preflight (204, no
  CORS headers, handler not run), disallowed-origin NON-preflight (`next()` called, no CORS headers
  added), and the no-Origin pass-through.

### 3.7 Security headers — set on response before `next()`, overwrite

- **Decision:** The middleware sets each configured header on `ctx.response` via
  `header(name,
  value)` **before** calling `next()`, so the value persists on the shared response
  builder through the handler and any short-circuit downstream. Configurable headers:
  `Content-Security-Policy`, `Strict-Transport-Security`, `X-Frame-Options`,
  `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`. The default set (applied when
  `headers` is omitted) is exactly:
  - `X-Content-Type-Options: nosniff`
  - `X-Frame-Options: DENY`
  - `Referrer-Policy: no-referrer`
  - `Strict-Transport-Security: max-age=31536000; includeSubDomains`

  `Content-Security-Policy` and `Permissions-Policy` have **no default** — they are emitted only
  when explicitly configured, because any default CSP/permissions policy breaks real apps in
  app-specific ways. `header()` overwrites any prior value; a per-header `false` in the options
  omits that header from the default set (`undefined` = keep the default).
...
- **Test home:** `test/unit/security-headers-middleware.test.ts` asserts the four named default
  headers field-by-field (values above, and CSP/Permissions-Policy ABSENT by default), that a custom
  value overrides a default, that per-header `false` omits it, that `enabled: false` omits all, and
...
- **Test home:** `test/unit/request-size-middleware.test.ts` and `csrf-middleware.test.ts` assert
  the exact status + `{ error, message }` body.
...
| `CorsOptions.enabled` / `CsrfOptions.enabled` / `RequestSizeOptions.enabled` / `IpSecurityOptions.enabled`                                                    | each concern's own factory (§3.2 — the ONLY place the flag is read) | Default `true`; `false` makes the factory return a pass-through middleware, so plugin and standalone entry points share one implementation.            |
...
| `CorsOptions.methods` / `allowedHeaders` / `exposedHeaders` / `maxAge`                                                                                        | `corsMiddleware`                                                    | Preflight `Access-Control-Allow-Methods`/`Access-Control-Allow-Headers`, response `Access-Control-Expose-Headers`, preflight `Access-Control-Max-Age`. |
| `CsrfOptions.trustedOrigins`                                                                                                                                  | `csrfMiddleware`                                                    | Additional scheme+host origins trusted on unsafe methods, default `[]`; the request's own origin is always implicitly trusted (§3.3).                  |
| `CsrfOptions.customHeader`                                                                                                                                    | `csrfMiddleware`                                                    | When set, unsafe methods must carry this header or the request is rejected 403.                                                                        |
| `RequestSizeOptions.maxBodySize`                                                                                                                              | `requestSizeMiddleware`                                             | Byte ceiling compared against the `Content-Length` header; over → 413. Default `1_048_576` (1 MiB, §3.4).                                              |
...
| ---------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `test/unit/http-security-plugin.test.ts`             | `src/plugin/http-security-plugin.ts`            | Default options → only security-headers middleware active (observed via `inject()`); each opt-in block → its concern active; `enabled: false` on a present block → concern inactive; one concern driven through BOTH entry points (plugin + standalone factory) under the same non-default options with identical behavior; plugin `name === 'http-security-plugin'`, `version === '0.1.0'`, no `provides`; `register()` does not throw with empty options. |
| `test/unit/cors-middleware.test.ts`                  | `src/middleware/cors-middleware.ts`             | Each `origin` variant (bool/string/array/fn) allow/deny; credentials reflection + no `*`; preflight (`OPTIONS`+`Origin`+`ACRM`) allowed → 204 + `Access-Control-Allow-*` headers and `next()` NOT called; preflight disallowed → 204 no CORS headers, `next()` NOT called; non-preflight disallowed origin → `next()` called, no CORS headers added; no `Origin` → pass-through `next()` called; `Vary: Origin` appended.                                   |
| `test/unit/security-headers-middleware.test.ts`      | `src/middleware/security-headers-middleware.ts` | The four named default headers (§3.7 values) asserted field-by-field, CSP/Permissions-Policy ABSENT by default; custom value overrides default; per-header `false` omits it; `enabled: false` → none; headers persist when downstream short-circuits.                                                                                                                                                                                                       |
| `test/unit/csrf-middleware.test.ts`                  | `src/middleware/csrf-middleware.ts`             | Unsafe + same-origin `Origin`, no `trustedOrigins` → pass (implicit self-trust); unsafe + origin in `trustedOrigins` → pass; unsafe + cross-origin outside trusted set → 403 `{error,message}` + handler not run; `Referer` fallback when `Origin` absent; unsafe + BOTH headers absent (empty AND non-empty allowlist) → pass; safe method → pass; `customHeader` absent on unsafe → 403; present → pass.                                                  |
| `test/unit/request-size-middleware.test.ts`          | `src/middleware/request-size-middleware.ts`     | `Content-Length` > `maxBodySize` → 413 `{error,message}` + `next()` not called; ≤ → pass; absent `Content-Length` → pass; non-numeric/negative `Content-Length` → pass; 1 MiB default applies when `maxBodySize` omitted.                                                                                                                                                                                                                                   |
...
```bash
git branch --show-current   # MUST be feat/m17-http-security-plugin, never main
...
point "Next milestone" at 18, and (in the same PR)
`git mv plans/milestone-17-http-security-plugin.md
plans/archive/`.
...
- **Risk:** Origin/Referer-based CSRF can be bypassed by an attacker who controls a trusted origin,
  or by a client that sends NEITHER header on an unsafe request (which passes through by design,
  §3.3 — browsers always send `Origin` on cross-origin unsafe requests, so the pass-through only
  admits non-browser clients, which CSRF does not target). **Mitigation:** Documented limitation;
  the optional `customHeader` requirement adds defense-in-depth; a cookie synchronizer is deferred
  until `IRequest` gains cookie access (§9).
- **Risk:** `trustProxy: true` with a spoofable `X-Forwarded-For` could let a client forge the
  ...
  warning).
- **Risk:** Default-on security headers could change an app's existing header behavior.
  **Mitigation:** `headers: { enabled: false }` disables the whole set and per-header `false` omits
  individual headers; documented in PUBLIC_API + README.
...
- **IP allow/deny lists and geo policy** — deferred; this milestone resolves and publishes the
  client IP only. Blocking policy is a later addition to this package (no milestone assigned yet).
