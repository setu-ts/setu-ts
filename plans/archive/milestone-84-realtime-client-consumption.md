...
| `apps/realtime-clients/*`                          | Example app whose `smoke` task drives both clients on Deno, Node, Bun and workerd (§3.6). Outside the workspace, own `deno.json` and NO `package.json` — measured: Node `--experimental-strip-types` and Bun both load `driver.ts` without one, while carrying one switches Deno to node_modules resolution and breaks the app's own cold type-check (M37c). |
