...
| `packages/kernel/test/unit/inject-body-shapes.test.ts` (new)             | `application/application.ts`                          | One table over `string`, `Uint8Array`, `ArrayBuffer`, `Blob`, `URLSearchParams` and a plain object: each asserts the exact bytes `await ctx.request.bytes()` yields, so the `{}`-for-everything outcome (M5) cannot return. Plus the named refusal, driven with a REPRESENTATIVE non-plain object rather than one unspecified case (added after review, PR #309, CodeRabbit): a `Date` and a class instance each refuse, beside the plain-object case that still serializes — without which a broad `typeof body === 'object'` runtime predicate passes the row while JSON-stringifying both. **The type already refuses all three** — probed: `Date`, a class instance and an array are each rejected by `Record<string, unknown>`, with the `@ts-expect-error` directives satisfied rather than unused — so what this row tests is the RUNTIME check §3.9 requires, which is the half a JavaScript or `unknown`-typed caller reaches. And the PER-SHAPE content-type defaults §3.9 specifies — `URLSearchParams` → `application/x-www-form-urlencoded`, a plain object → `application/json`, every byte-ish shape → none. (Corrected after review, PR #309 finding 10: this row previously said "the way the string path does", which contradicts §3.9 — the string path defaults to JSON, so implementing the row literally would have made `formData()` refuse the body the widening exists to serve.) |
...
**Coverage.** Corrected after review (PR #309, CodeRabbit) — this paragraph predated §3.9 and named
three files where there are four. `packages/kernel/src/application/application.ts` gains the most
branches in the letter: one arm per body shape, the per-shape content-type defaults, and the named
refusal. They are covered by `inject-body-shapes.test.ts` (every arm, asserted on the bytes
`ctx.request.bytes()` yields) and `inject-multipart-form.test.ts`. The other three are
`csrf/token.ts`, `csrf-form-middleware.ts` and `multipart-parser.ts`; every new arm in all four is
reachable from a documented entry point, so the tables above take each to the per-file bar.
`cache-control.ts` and `static-handler.ts` gain no branch — a rename and comments only — so their
existing numbers must not move, which is itself checked by re-reading the per-file table after the
change.
