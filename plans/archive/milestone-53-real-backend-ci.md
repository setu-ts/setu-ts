...
- **In scope:**
  ...
  - `scripts/check-apps.ts` — track skipped examples by name, make a skip for a provided backend a
    ...
    (deliverables 3, 6).
  - `deno.json` (root) — add `--allow-env=ALLOW_SKIP` to `check:apps`, which otherwise cannot read
    its own new env var. The `test` / `test:coverage` strings are left alone (§3.4).
  - `packages/{cache,messaging,queue}-plugin/deno.json` — an endpoint-scoped `test.permissions.net`
    so the deepened guarded Redis tests can open a TCP connection to the service container in CI.
...
| `test` / `test:coverage` tasks            | `deno.json:54` / `deno.json:55`                                                                                  | Grant `--allow-read --allow-import --allow-env --allow-sys=hostname` but **NOT `--allow-net`**. Probed with `Deno.permissions.query({name:'net'})` inside all three packages: `net=prompt` (denied). The deepened tests need a net grant, which §3.4 places in each package's own `test.permissions` rather than here — a root flag REPLACES a package block's net value, narrowing the nine packages that already declare `net: true` and breaking 21 of their tests (measured).                                                                                                                                                                                                                                                                             |
...
| `-P` flag meaning                         | `deno test --help` (Deno 2.9.4)                                                                                  | `-P, --permission-set[=<NAME>]` — **not** `--parallel`. Package manifests carry their own `test.permissions` blocks (`packages/cache-plugin/deno.json:7` has one; `queue-plugin` and `messaging-plugin` have none), so the root task string is not the only permission source. Probed both ways: an unscoped `--allow-net` on the CLI yields `net=granted` in cache-plugin (which has a block) _and_ in queue/messaging (which do not) — but a SCOPED `--allow-net=<list>` REPLACES a package's `net: true` rather than unioning with it, which is what broke 21 GraphQL/SSE/WebSocket e2e tests when tried. Nine packages already declare `test.permissions.net`, so the per-package block is the established convention and the correct home for the grant. |
...
- **Decision:** In `.github/workflows/ci.yml`, add a `services:` block to the `deno` job running
  ...
  `Example
  applications` step. In `deno.json`, make two permission edits: add an endpoint-scoped
  `test.permissions.net` to the three Redis packages' manifests, and add `--allow-env=ALLOW_SKIP` to
  `check:apps`.
...
- **Why net access is required:** the three Redis packages grant no network permission today
  (probed: `net=prompt`), so a guarded test that sees `REDIS_URL` set in CI would throw
  `PermissionDenied` on connect — and since the guard only checks "url set + ioredis imports", that
  surfaces as a hard failure rather than a skip, defeating the gate. `--allow-import` (already
  present) covers fetching `npm:ioredis@5.x`; the net grant covers the resulting TCP socket.
- **Where the grant goes: each package's own `test.permissions`, scoped to the Redis endpoint.**
  `packages/{cache,messaging,queue}-plugin/deno.json` each declare
  `"net": ["127.0.0.1:6379", "localhost:6379"]`. This is the repo's established convention, not an
  invention — **nine** packages (`graphql`, `grpc`, `logger`, `react-router`, `realtime-backplane`,
  `runtime`, `service-discovery`, `sse`, `websocket`) already carry a `test.permissions` net grant.
  An earlier draft put a scoped `--allow-net` on the root `test` / `test:coverage` tasks instead;
  that is **wrong and was measured wrong**. A CLI `--allow-net=<list>` does not union with a
  package's block — it REPLACES it, so scoping at the root narrowed all nine of those packages from
  `net: true` down to the Redis endpoint and broke **21** tests (GraphQL HTTP e2e, real
  subscriptions, SSE, security — every suite that binds an ephemeral port). The root task strings
  are therefore left untouched.
- **Why scoped to the endpoint rather than `"net": true`.** A blanket grant lets those three
  packages, and their transitive `npm:` dependencies, reach any host during `deno test`. Scoping is
  the tighter CI-supply-chain posture at no cost, and it also makes §3.6's tests deterministic. It
  changes nothing for consumers: a package manifest ships in the JSR tarball, but a dependency's
  `test.permissions` cannot leak to a consuming application — probed from a separate project under
  both `deno run` and `deno test`, each reporting `net = prompt` with a real connect blocked as
  `NotCapable`.
- **A loopback-wide grant was also tried and rejected, for a measured reason.**
  `127.0.0.1,localhost` (any port) would let §3.6's tests observe a real `ECONNREFUSED` rather than
  a permission error. It **hangs the test runner**: ioredis's default `retryStrategy` reconnects
  forever, so a refused connect leaves a retry loop holding the event loop open — measured, both
  tests reported `ok` and the process then ran until killed at 110 s, emitting
  `[ioredis] Unhandled error event: connect ECONNREFUSED` throughout. Endpoint scoping avoids that
  entirely: an unpermitted address fails synchronously with `NotCapable` before any socket exists,
  so there is nothing to retry. This is why §3.6's tests are pointed at an address the grant does
  not cover rather than at a merely-closed one.
...
### 3.6 Two existing tests that passed only because Redis was unreachable

- **Decision:** `cache-plugin/test/unit/cache-plugin.test.ts` ("throws when redis client cannot be
  resolved") and `queue-plugin/test/unit/queue-plugin.test.ts` ("drops wrong-shaped redis client and
  falls through to lazy-load") both assert `rejects.toThrow()` against `redis://localhost:6379` —
  the default, and exactly the address CI now serves. Each is repointed at `redis://127.0.0.1:6390`,
  an address §3.4's port-scoped `--allow-net` deliberately does NOT cover, so the connect fails
  synchronously with `NotCapable` whatever is listening on the host. The rejection therefore arrives
  as a permission denial rather than an `ECONNREFUSED` — that is the design, not a compromise: the
  refusal variant hangs the runner (§3.4), and the code path under test (wrong-shaped client
  dropped, lazy-load entered, connect attempted) is identical in both cases. What the old form could
  not do, and this one cannot fail to do, is reject when a real Redis happens to be reachable.
- **Why:** these tests were getting their rejection from an absent backend rather than from the
  behaviour they name, so providing a real Redis flips them from green to red. Measured: with Redis
  stopped, `3 passed | 0 failed`; with Redis running, `1 passed | 2 failed`. The queue test's own
  comment admitted the dependency ("Since we don't have a real Redis, the lazy-load will fail at
  connect time"). This is the milestone's thesis applied to itself — the same pass-for-the-wrong-
  reason defect it was written to catch — so the fix is to make them fail honestly, not to exclude
  them from the Redis-enabled run.
- **Test home:** the tests themselves. Verified under both conditions: they pass with a live Redis
...
| `test.permissions.net` (per-package manifest) | `deno test` under `cache-plugin`, `messaging-plugin`, `queue-plugin`              | Permits the TCP connect the deepened tests need, to the Redis endpoint and nothing else; no effect when `REDIS_URL` is unset (tests skip before connecting). Set per package, never as a root task flag — a root flag replaces rather than unions (§3.4). |
...
| `deno.json`                                                     | Add `--allow-env=ALLOW_SKIP` to `check:apps` (`:52`). The `test` / `test:coverage` strings are deliberately UNCHANGED (§3.4).                                                                                                         |
| `packages/{cache,messaging,queue}-plugin/deno.json`             | Declare `test.permissions.net` scoped to `127.0.0.1:6379` + `localhost:6379` (§3.4).                                                                                                                                                  |
...
| `packages/cache-plugin/test/unit/cache-plugin.test.ts`          | Repoint the redis-resolution test at a closed loopback port so it no longer depends on 6379 being dead (§3.6).                                                                                                                        |
| `packages/queue-plugin/test/unit/queue-plugin.test.ts`          | Same for the wrong-shaped-client drop test (§3.6).                                                                                                                                                                                    |
...
deno task lint
deno task check
deno task test              # guarded Redis tests run when REDIS_URL is set, skip otherwise
...
- **Granting net changes what EXISTING tests do — in two different ways, both measured.** An early
  draft claimed the grant "does not create new calls"; false. (a) `redis-queue.test.ts:497` and
  `redis-streams-broker.test.ts:598` connect to `redis://localhost:9999` and assert a rejection.
  Under the endpoint-scoped grant that address stays unpermitted, so they still reject at the
  permission boundary with no socket — unchanged, and deliberately so. (b) The two tests in §3.6
  targeted `localhost:6379`, which a live Redis answers, so they flipped from green to red the
  moment CI provided one. Mitigation: scope the grant to the Redis endpoint (§3.4), point §3.6's
