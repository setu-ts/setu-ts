...
| `status()` validates NOTHING, and the throw is downstream        | `packages/kernel/src/context/response.ts:36-39,87-93`                                                                        | `status(code)` is a bare `this.#status = code`; `redirect(url, status = 302)` likewise. Measured on Deno 2.9.6: `new Response('x', { status: 999 })` and `{ status: 99 }` each throw `RangeError: … outside the range [200, 599]`, inside the adapter's mapping after the pipeline has finished. `error-responder.ts:255-295` records this same class for three shipped application-authored statuses; `@HttpCode` would be a fourth (§3.3a). |
| A body at `204`/`205`/`304` is already dropped, not thrown       | `packages/runtime/src/adapters/shared/fetch-mapping.ts:424,453-462`                                                          | `NULL_BODY_STATUSES` is `{204, 205, 304}`; a body written at one of them is dropped and a stream body cancelled. Its own JSDoc records that before the guard existed all three threw `TypeError: Response with null body status cannot have body` out of `app.fetch`. Re-measured here. So `@HttpCode(204)` needs no new guard — §3.3a cites this rather than duplicating it.                                                                 |
...
### 3.3a Which status values `@HttpCode` and `@Redirect` accept — refused at startup, never clamped

- **Decision:** `@HttpCode(status)` and `@Redirect(url, status?)` refuse, from `register()` when the
  route is built, any status that is not an integer in `[200, 599]`, naming the class, the method
  and the offending value. `@Redirect` additionally refuses a status outside `[300, 399]`. Nothing
  is clamped and nothing is silently corrected.
- **Why a refusal at all:** `ResponseBuilder.status(code)` is a bare assignment with no validation
  (`packages/kernel/src/context/response.ts:36-39`), and `IResponse.redirect(url, status = 302)` is
  the same (`:87-93`). The web `Response` constructor is what rejects the value, far downstream:
  measured on Deno 2.9.6, `new Response('x', { status: 999 })` and `{ status: 99 }` each throw
  `RangeError: The status provided (…) is not equal to 101 and outside the range [200, 599]`. That
  throw happens inside `mapSnapshotToWebResponse` after the pipeline has finished, which
  `packages/common/src/errors/error-responder.ts:255-295` already records as the failure shape for
  this exact class — it names three shipped call sites that let an APPLICATION author a status
  (`FlagGuardOptions.statusCode`, the multi-tenancy `rejectionStatus`, a
  `WebSocketGuardDecision.status`) and says "a plausible typo such as `4004` for `404` crashes every
  request to that route". `@HttpCode(status: number)` would be a **fourth** such site and the first
  on the success path, so the milestone must answer it rather than inherit it.
- **Why refuse rather than clamp, unlike `serveableStatus`:** that helper clamps to `500` because
  "the caller definitely wants an error response and only the number is wrong" — a per-request
  recovery on a path that must stay answerable. `@HttpCode` is the opposite case: the value is a
  compile-time literal known when the route is registered, there is no request in flight to rescue,
  and clamping a success status to `500` would turn a typo into a served error. A startup refusal is
  both earlier and more precise, and it matches §3.3's, §3.5's and M97a's use of the same mechanism.
  `serveableStatus` is not reused: it is module-private to `error-responder.ts`, it clamps rather
  than reports, and its null-body arm is wrong here (see below).
- **Bodyless statuses (`204`/`205`/`304`) need NO new rule, and the plan says so rather than
  inventing one.** `@HttpCode(204)` on a handler returning a plain value is safe today:
  `mapSnapshotToWebResponse` drops a body written at one of `NULL_BODY_STATUSES`
  (`packages/runtime/src/adapters/shared/fetch-mapping.ts:424,453-462`), a guard whose own JSDoc
  records that before it existed all three threw
  `TypeError: Response with null body status cannot
  have body` out of `app.fetch` — measured, and
  re-measured here. So a `204` is the conformant bodiless response RFC 9110 §15.3.5 asks for, which
  is exactly what a `DELETE` handler wants and the second most likely `@HttpCode` argument after
  `201`. Adding a second guard in this package would be §11.1 duplication of a decision already made
  one layer down; what this milestone owes is the citation, so an implementer does not write the
  redundant check and a reader knows the case works. A stream body at those statuses is cancelled
  rather than abandoned by the same guard.
- **Why `@Redirect` narrows to `3xx` while `@HttpCode` does not:** `@HttpCode` is deliberately not
  restricted to `2xx` — a handler answering `404` through a decorator is legitimate, and
  `HttpError`/`errorHandler` own the error path but do not monopolise the status space. `@Redirect`
  is named for one thing, sets `Location`, and a non-`3xx` value makes the pair self-contradictory:
  `@Redirect(url, 200)` would serve a `200` carrying a `Location` no client follows, which is a
  silent no-op of the whole decorator. That is the dead-surface shape, so it is refused.
- **Test home:** `packages/decorator-plugin/test/unit/response-status-validation.test.ts` — the
  refused values (`0`, `99`, `600`, `1000`, `2.5`, `NaN`, `4004`) each named in the message, one
  accepted value per boundary (`200`, `599`), `@Redirect` refusing `200` and `400` while accepting
  `301`/`302`/`307`/`308`, and a `@HttpCode(204)` case driven through a REAL kernel application
...
| `@HttpCode(status)` argument                             | `createHandler` → `ctx.response.status(...)`.                                        | The success status for a plain return. Must be an integer in `[200, 599]` or `register()` refuses (§3.3a). Overridden by a returned `HandlerResult` (§3.2). `204`/`205`/`304` serve bodiless — the mapper drops the body, no guard needed here. |
| `@Redirect(url, status?)` arguments                      | `createHandler` → `status` + `Location` header.                                      | `status` defaults to `302`, matching `IResponse.redirect`'s documented default (`common/src/http.ts:248`), and must be an integer in `[300, 399]` or `register()` refuses (§3.3a).                                                              |
