...
### 3.2 One `TracedQueue` decorator, applied in the plugin, never per adapter

- **Decision:** `queue-plugin/src/tracing/traced-queue.ts` wraps the resolved `QueueAdapter` the way
  `TracedBroker` wraps a `MessageBrokerAdapter`, and `QueuePlugin` applies it when
  `CAPABILITIES.TELEMETRY` is registered. It injects `traceparent` into `AddJobOptions.headers` on
  `add`, and on dispatch it reads `IJob.headers` and starts the handler span as a child of the
  extracted context. **No adapter contains any tracing code.**
- **Why:** the adapters' job is to carry an opaque map, and putting extraction in four places is
  four chances to disagree about a header name — the split that produced X31-1 in `static-plugin`
  and the duplicate-conditional defect in M90e. The decorator shape is already proven here
  (`traced-broker.ts`), which also means a reviewer comparing the two files can see they agree.
...
| `queue-plugin/src/tracing/traced-queue.ts`                       | The decorator.                                                                                |
| `queue-plugin/src/plugin/queue-plugin.ts`                        | `optionalDependencies` gains `CAPABILITIES.TELEMETRY`; the decorator is applied when present. |
...
| `queue-plugin/test/unit/traced-queue.test.ts` (new)                           | `tracing/traced-queue.ts`          | `add` injects a well-formed `traceparent` under a span; dispatch parents the handler span to the extracted context; a job with **no** headers starts a root span and does not throw; a telemetry service that throws leaves the enqueue working.                              |
