...
rows (E1, E2) are this milestone's substance. It then closes the seventeen remaining alpha.8
register rows that no other M70 workstream absorbs, and recommends an eighteenth (X2-6) be
reassigned.

**A scope finding the ROADMAP line does not record, raised here rather than absorbed silently.** The
line calls the twenty assigned rows "mechanical documentation rows". Source-checking every one says
otherwise: **ten are code changes**, spanning `static-plugin`, `auth-plugin`, `session-plugin`,
`audit-plugin`, `react-router-plugin`, `realtime-backplane-plugin`, `common`, `sse-plugin`,
`messaging-plugin` and `starters`; three of those widen a committed `common` contract. Two more (C1,
X8-8) are already closed by merged milestones and must not be re-done, leaving seventeen. The plan
below carries the full assigned scope with each row's true nature named, and §9 records the one row
this plan recommends the maintainer reassign rather than land here (X2-6, broker trace propagation,
which is a feature milestone wearing a register row's clothes).
...
| `audit-plugin` barrel                   | `packages/audit-plugin/src/index.ts:12-39`                                                                    | Exports the four storage classes and `IAuditDbClient`, but neither `StoredAuditEntry` nor `AuditQuery`. Both are declared at `packages/audit-plugin/src/interfaces/index.ts:21,38`, so X4-7 is two type exports and NOT a `common` widening. The exported classes' `query` signatures are unnameable. |
...
| -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| C1 | `ROADMAP.md:7195` assigns C1 and X8-8 to this milestone; both are closed in merged `main` (M70m PR #181, M70k PR #178). `smoke/DEFECTS.md:58,117` still says `open` and `closed · M70k` respectively.                 | The source wins. C1 and X8-8 are not re-done. The register's C1 row is corrected to `closed · M70m`; the ROADMAP line is corrected to stop assigning them.                                      | **Shipped.** `ROADMAP.md` M70n row list drops C1 and X8-8 and names the closing PRs; `smoke/DEFECTS.md` C1 status still to correct.                    |
| C2 | `ROADMAP.md:7195` calls the assigned rows "mechanical documentation rows"; ten are code changes and three widen `common`. Its "Scope realities" bullet also lists X4-7 as a `common` widening, which §1 disproves.    | The source wins on both. The M70n row is rewritten to describe what the rows are; the scope bullet's X4-7 claim is struck and replaced with M70n's three real widenings.                        | **Shipped.** `ROADMAP.md` M70n row rewritten with the code/doc split and the X2-6 recommendation; scope bullet corrected; progress-table row retitled. |
