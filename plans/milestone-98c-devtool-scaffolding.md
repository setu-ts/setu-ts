...
- **Decision:** `scripts/dev.ts` spawns `main.dev.ts` for a member carrying `devtoolPort` and
  `main.ts` for every other member, reading `setu.workspace.json` as it already does
  (`dev-runner.ts:43`, `:120`). The root `dev` task's grant is unchanged: it already carries
  `--allow-net` unscoped (`runtime-profile.ts:142`).
