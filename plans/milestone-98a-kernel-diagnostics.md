...
| kernel `integration/diagnostics-fault-isolation.test.ts`     | collector/application/lifecycle      | A throwing injected clock and direct private event/topology `safeObserve` callbacks leave synchronous/async responses and lifecycle errors unchanged, disable later event capture, report only a saturated value-free drop count and mark failed topology truncated. |
