...
| `GeneratedFile.managed`    | `packages/cli/src/utils/file-writer.ts:16-35`                                                                                                                                                                                                                  | read ONLY by `findExisting`; exempts the aggregate barrel from the overwrite refusal                                |
...
- **Decision:** `writeFiles` records every path it writes and every directory it creates, and on any
  failure removes them in reverse order before rethrowing. Directories are removed non-recursively,
  so one that already held the developer's files is left alone.
- **Why:** rollback rather than idempotence, because `findExisting` already refuses a pre-existing
  path — so the retry path the developer needs is "as if the run never happened". The `created` set
  the function already maintains is half the bookkeeping. Non-recursive directory removal is the
  safety property: a directory the CLI created is empty at rollback time, and one it did not create
  is not in the set.
...
| ----------------------------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/cli/test/unit/write-files-rollback.test.ts` | `utils/file-writer.ts`            | a fake `IFileSystem` whose Nth `writeFile` rejects leaves NO file behind and removes only directories it created; a pre-existing directory survives; a rejecting `rm` reports both errors. Calls typed against `writeFiles(fs: IFileSystem, files: readonly GeneratedFile[])` |
...
| `packages/cli/test/e2e/scaffold-interrupted.test.ts`  | `utils/file-writer.ts` end to end | `setu new` into a target whose nested directory is read-only leaves nothing behind, and an immediate retry SUCCEEDS. Same for `setu generate module` with a read-only `managed` barrel                                                                                        |
...
3. Remove the rollback — `scaffold-interrupted` fails on the retry for both `new` and `generate`.
4. Widen `adopt`'s catch back — `adopt-entry-rewrite` reports a write failure as a port-literal
   miss.
...
- Rollback could remove a file a concurrent process created at the same path → the set holds only
  paths this invocation wrote, and `findExisting` already refused pre-existing paths before the
  first write.
