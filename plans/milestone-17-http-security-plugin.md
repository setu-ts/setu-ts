...
- **Decision:** `origin` accepts `boolean` (`true` = reflect the request Origin, `false` = deny
  ...
  `Allow-Methods`/`Allow-Headers`/`Max-Age` (from options) and **short-circuits (no `next()`)**; on
  a disallowed origin it responds 204 with no CORS headers (also short-circuiting). Non-preflight
  requests with an allowed Origin get CORS headers added then call `next()`; non-preflight requests
  with a DISALLOWED Origin call `next()` with no CORS headers added (the browser enforces the block
  — the server does not reject); requests with no `Origin` are not CORS requests and pass through
  untouched.
...
| `test/unit/cors-middleware.test.ts`                  | `src/middleware/cors-middleware.ts`             | Each `origin` variant (bool/string/array/fn) allow/deny; credentials reflection + no `*`; preflight (`OPTIONS`+`Origin`+`ACRM`) allowed → 204 + `Allow-*` headers and `next()` NOT called; preflight disallowed → 204 no CORS headers, `next()` NOT called; non-preflight disallowed origin → `next()` called, no CORS headers added; no `Origin` → pass-through `next()` called; `Vary: Origin` appended.                                                  |
