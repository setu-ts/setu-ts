...
- **In scope:** `CAPABILITIES.REDACTION` and the `IRedactionService` port in `common`; the pure
  ...
  service applied after normalisation); `telemetry-plugin` (the `http.url` leak repair and
...
| `IRedactionService`             | interface | `ConsoleLogger`/`PinoLogger` (constructor), `AuditService`, `sanitizeUrl`; the type `RedactionPlugin` registers.                                                                                                                                                                                                                                                                        |
...
No `src/index.ts` change in `logger-plugin`, `telemetry-plugin` or `audit-plugin` — `sanitizeUrl` is
internal and no logger type changes. Each of the three gains a `barrel-exports.test.ts` assertion
pinning that (the M56 defect class: a barrel change that no test sees).
...
- **Two redaction passes run at one insertion point.** `redact` and the service both apply inside
  ...
  both name.
- **The 11 new `common` exports can move the JSDoc ratchet.** M90a had to CUT two exports because
