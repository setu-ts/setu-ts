...
is a mechanism, not a compliance feature: standard classification identifiers (`pii`, `phi`, `pci`)
name field-handling intent, while no regulation is named in any shipped identifier, doc line or
option. The recurrence grep therefore excludes only regulation names.
...
| C5 | An earlier draft proposed an `@setu-ts/redaction-plugin`, while `ARCHITECTURE.md` §8 lists the package graph.                                                                                         | No diagram node or package edge is added: the selected design has no new package, token, or registration. `common` owns the pure implementation and the three consumers receive it through existing plugin options. | None. The `ARCHITECTURE.md` §14 responsibility rows remain the named documentation deliverable.                                                                                                                                                                                                                                                                                              |
...
| --------------------------------------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `common/test/unit/redaction-service.test.ts`              | `redaction/field-matcher.ts`                                       | Literal dot-path; `*` matches exactly one segment including an array index; `**` matches zero or more; case-insensitive segments; a non-matching path returns `undefined`; a pattern longer than the path does not match.                                                                                                                                                                                                                                               |
| `common/test/unit/redaction-service.test.ts`              | `redaction/redactors.ts`                                           | `eraseRedactor` → `'[Redacted]'` for every input type; `createMaskRedactor({ keep: 4 })` preserves exactly four trailing characters; invalid/negative/non-finite `keep` fails closed; non-string input erased; a custom redactor receives both `context.path` and `context.classification` (§4's only reader). Calls type-check against `Redactor`.                                                                                                                     |
...
| `logger-plugin/test/unit/console-logger.test.ts`          | `loggers/console-logger.ts`                                        | Regression: the caller's nested object is **byte-identical after logging**; wildcard and array paths redact; a policy and `redact` agree on normalized nested errors; a path named by both is erased; `child()` carries the service; `undefined` metadata passes through.                                                                                                                                                                                               |
| `logger-plugin/test/integration/logger-plugin.test.ts`    | `plugin/logger-plugin.ts`                                          | With no `redaction` option the secret default is active; `redact: []` restores prior behavior; both option arms resolve through the real plugin.                                                                                                                                                                                                                                                                                                                        |
| `logger-plugin/test/unit/pino-logger.test.ts`             | `loggers/pino-logger.ts`                                           | The service is applied to log metadata, retained base bindings, and child bindings; `redact` still reaches Pino's options (guarded real-import test for the Pino path).                                                                                                                                                                                                                                                                                                 |
...
| `telemetry-plugin/test/unit/telemetry-middleware.test.ts` | `middleware/telemetry-middleware.ts`, `plugin/telemetry-plugin.ts` | A kernel-equivalent middleware exercise verifies an `http.url` with policy-transformed query values, repeated parameters intact, and the plugin warning fires exactly once without a policy.                                                                                                                                                                                                                                                                            |
| `audit-plugin/test/unit/audit-service.test.ts`            | `services/audit-service.ts`                                        | `before`/`after`/`metadata` are each redacted; redaction precedes the freeze (the stored record is frozen **and** redacted); an absent field stays absent; with no service the stored record is byte-identical to today.                                                                                                                                                                                                                                                |
| `audit-plugin/test/integration/audit-integration.test.ts` | `plugin/audit-plugin.ts`                                           | Real app with `AuditPlugin({ redaction })`: an entry written through the resolved `IAuditLogger` reads back redacted from a configured sink; BOTH option arms drive the same result (§3.4).                                                                                                                                                                                                                                                                             |
...
| `test/unit/compliance-vocabulary.test.ts` (root)          | (recurrence gate)                                                  | `grep`-equivalent over `packages/*/src`: standard classification identifiers PII/PHI/PCI remain permitted, while no shipped identifier, option or doc line names HIPAA, GDPR or DSS — the §0 scope boundary made mechanical.                                                                                                                                                                                                                                            |
