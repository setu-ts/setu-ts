...
| `IngressContext`                | `common/src/services/ingress.ts:43-73`                      | `kind`, `name`, `payload`, `attempt?`, `headers?` (messaging arm only). Explicitly **no `state`, no `services`**: "a behaviour needing a capability closes over it via its `RegistryFactory` arm".                                                                                                      |
| `IIngressBehavior`              | `common/src/services/ingress.ts:112-123`                    | One method: `handle(ctx, next): void | Promise<void>` — return without calling `next` to short-circuit.                                                                                                                                                                   |
| `IMultiTenancyService`          | `common/src/services/tenancy.ts:51-78`                      | `getCurrentTenant(ctx: IRequestContext)`, `getRepository<E,I>(ctx: IRequestContext, entity)`, `prefixCacheKey(tenantId, key)`. **Only the last is ctx-free.**                                                                                                          |
...
| `prefixCacheKey`'s own JSDoc    | `common/src/services/tenancy.ts:66-77`                      | "The separator is deliberately NOT a per-call argument: this method is the single home for separator resolution" — the precedent for a ctx-free, id-taking member.                                                                                                     |
...
| -- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| C1 | `CHANGELOG.md`'s `0.3.0` entry says the ingress chain lets "an authorization, **tenant** or audit concern" be expressed once per ingress kind. Measured, the tenant one cannot be written at all.                                                        | The **claim** is the thing to fix, and this milestone makes it true rather than retracting it — §3.4 adds the ctx-free member. The CHANGELOG entry for `0.3.0` is history and stays; the new entry states what changed. | `CHANGELOG.md` gains the `common` addition and says plainly that the tenant concern became expressible in this release, not `0.3.0`. |
...
### 3.1 X16-1's fix: the in-memory broker resolves on invocation, not on completion
...
### 3.4 X16-2's fix: a ctx-free member on `IMultiTenancyService`
...
- **`tenantById` was in the first draft of this plan and is CUT, because nothing can implement it.**
  ...
  `findAll`/`findById`/`find`/`create`/`update`/`delete` (`interfaces/index.ts:162-196`) — with **no
...
- **Cost:** **breaking for out-of-repo implementors** of `IMultiTenancyService`. The framework's own
  service is the only in-repo implementor; CHANGELOG migration text names the added member. §9's
...
- **Test home:** the EXISTING `common/test/unit/ingress-contract.test.ts` is extended — it already
  pins the envelope's member set at compile time (`kind`/`name`/`payload`/`attempt?`/`headers?`);
  the added assertion is that NO tenant slot exists, so a later addition is deliberate. A second
  shape file would duplicate it (§11.1).
...
| `MessagingCommonOptions.chainReadyTimeoutMs` | option | Read by `messaging-plugin.ts` when constructing `PipelinedBroker`; see §4.1.                                                                                  |
| `InMemoryBrokerOptions`                      | type (NEW) | The `memory` arm at `messaging-plugin.ts:192` — `new InMemoryBroker(ctx.runtime, serializer, options)`; carries `onDispatchError` (§4.1). No such type exists today: the constructor takes `(runtime, serializer)` alone. |

§3.1's promise change adds **no** symbol; §3.1b adds `InMemoryBrokerOptions` (row above). `barrel-exports.test.ts` in `common`,
...
| -------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MessagingCommonOptions.chainReadyTimeoutMs` | `messaging-plugin.ts` → `new PipelinedBroker(...)`  | Bounds a held dispatch (default 10 000). `0` disables the bound and restores the pre-M89c wait-forever behaviour, for an application that would rather hang than fail. Ignored entirely when no behaviour factory is configured, because the gate is then `undefined`. Declared on `MessagingCommonOptions` (`interfaces/index.ts:192`) — the shared base every union arm extends, the `subscriptions`/`behaviors` precedent — never per-arm, since the gate exists for every broker.                                                                                                                         |
| No new tenancy option                        | —                                                   | Checked: §3.4 adds one member, not configuration.                                                                                                                                                                                                                                                                                                                                              |
| `InMemoryBrokerOptions.onDispatchError`      | `InMemoryBroker`'s retained-promise handler (§3.1b); the `memory` arm passes it at `messaging-plugin.ts:192` | The terminus of this broker's failure path (declared on a NEW `InMemoryBrokerOptions` — none exists today): called once per rejected handler with the error and the message metadata. Absent, the rejection is still observed and settled — never an unhandled rejection — then dropped, since this broker has no redelivery to fall back on. `MessagingPlugin` always supplies one, so the absent case is reachable only by constructing the broker directly. |

## 5. Implementation files

| File                                                                    | Purpose                                                                                                                                                                                       |
| ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/messaging-plugin/src/brokers/in-memory-broker.ts`             | `publish`/`publishWithHeaders` resolve on invocation while **retaining** each callback's promise; a NEW `InMemoryBrokerOptions` carrying `onDispatchError` (optional third constructor parameter); the `@returns` JSDoc restated, plus the no-redelivery asymmetry (§3.1b). |
...
| `packages/messaging-plugin/src/interfaces/index.ts`                     | The `chainReadyTimeoutMs` declaration on `MessagingCommonOptions` and its JSDoc.                                                                                                                                                           |
| `packages/messaging-plugin/src/index.ts`                                | Barrel re-export of `ChainGateTimeoutError`.                                                                                                                                                  |
| `packages/common/src/services/tenancy.ts`                               | `getRepositoryFor` — one member; §3.4 records why `tenantById` is cut.                                                                                                                        |
| `packages/common/src/services/ingress.ts`                               | C2's JSDoc sentence.                                                                                                                                                                          |
| `packages/multi-tenancy-plugin/src/services/*.ts`                       | The one member on the concrete service.                                                                                                                                                       |
...
| `PUBLIC_API.md`                                                         | `getRepositoryFor`, `ChainGateTimeoutError`, `InMemoryBrokerOptions`, `chainReadyTimeoutMs`, `onDispatchError`.                                                                                                        |
| `CHANGELOG.md`                                                          | Two breaking changes with migration text (the in-memory promise, the required member).                                                                                                       |

## 6. Test plan (every `src/` file mapped; per-file 90% bar)

| Test file                                                                 | src covered                                 | Key assertions (and the signature each call type-checks against)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `messaging-plugin/test/unit/in-memory-dispatch-timing.test.ts`            | `brokers/in-memory-broker.ts`               | `publish` resolves before a slow handler completes; the handler still runs; `publishWithHeaders` behaves identically (both against `IMessageBroker` in `common/src/services/messaging.ts`). **Rejected handlers:** a throwing handler reaches `onDispatchError`, `publish` still resolves, its siblings still receive the message, and **no `unhandledrejection` fires** (`addEventListener`, the only assertion that catches this failure mode). A second case asserts the promise the subscription callback RETURNS is the one whose rejection is observed, so a refactor that drops it fails here rather than in production. |
...
| `common/test/unit/ingress-contract.test.ts` (extend)                      | `services/ingress.ts`                       | The envelope's member set is exactly `kind`/`name`/`payload`/`attempt?`/`headers?` and carries NO tenant slot (§3.5), so a later slot is deliberate. The file already pins the shape at compile time; the addition is the tenant-slot absence.                                                  |
| `multi-tenancy-plugin/test/unit/ctx-free-members.test.ts`                 | `services/*.ts`                             | `getRepositoryFor` scopes writes to the id it is given, against the `common` signature from §1; two ids do not see each other's rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `common/test/unit/barrel-exports.test.ts` (extend), plus the two plugins' | the three `src/index.ts`                    | Only the §4 symbols moved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
