...
| `InMemoryBrokerOptions.onDispatchError`      | `InMemoryBroker`'s retained-promise handler (§3.1b) | The terminus of this broker's failure path: called once per rejected handler with the error and the message metadata. Absent, the rejection is still observed and settled — never an unhandled rejection — then dropped, since this broker has no redelivery to fall back on. `MessagingPlugin` always supplies one, so the absent case is reachable only by constructing the broker directly. |

## 5. Implementation files

| File                                                                    | Purpose                                                                                                                                                                                       |
| ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/messaging-plugin/src/brokers/in-memory-broker.ts`             | `publish`/`publishWithHeaders` resolve on invocation while **retaining** each callback's promise; `onDispatchError`; the `@returns` JSDoc restated, plus the no-redelivery asymmetry (§3.1b). |
...
| `PUBLIC_API.md`                                                         | `getRepositoryFor`, `ChainGateTimeoutError`, `chainReadyTimeoutMs`, `onDispatchError`.                                                                                                        |
