...
- **Decision:** `endpoint?: string | undefined` on `AwsKmsProviderOptions` and
  `GcpSecretManagerProviderOptions`, passed to the lazily-loaded client and ignored when a `client`
  is injected.
- **Why:** it matches `S3Provider`, which is the in-repo model, and matches Azure's existing
  `vaultUrl` in effect. Optional, so every existing configuration compiles and behaves identically.
  Ignored under an injected client because that client is already constructed — the same rule
  `region` follows today.
...
- **Test home:** the same test asserts the endpoint reaches the client unmodified.
...
| `GcpSecretManagerProviderOptions.endpoint` | option | `gcp-secret-manager.ts`'s lazy client construction; and the Pub/Sub-style emulator host an application supplies |
...
| ------------------------------------------ | ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `AwsKmsProviderOptions.endpoint`           | the lazy `@aws-sdk/client-secrets-manager` client | absent: SDK default resolution, unchanged. Present: passed as the client's endpoint. With `client` injected: ignored |
| `GcpSecretManagerProviderOptions.endpoint` | the lazy `@google-cloud/secret-manager` client    | same three arms                                                                                                      |
...
| `packages/secrets-plugin/src/providers/gcp-secret-manager.ts` | the same                                                          |
...
| `packages/secrets-plugin/test/unit/provider-endpoint.test.ts`        | `providers/aws-kms.ts`, `providers/gcp-secret-manager.ts` | a fake SDK module records the constructor argument: absent endpoint passes none, present passes it verbatim, injected client ignores it. Types check against the widened options interfaces                                                                          |
