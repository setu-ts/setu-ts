...
- **In scope:** a required `peek(name)` member on `IWebSocketService` and `ISseService` returning the
  live room/channel when one exists and `undefined` when none does, backed by a `peek` on each
...
- **NOT this milestone:** a cookie-reading auth strategy for the two realtime transports (M73, whose
  ...
  (§9 — a behaviour change that risks silent partial delivery, deliberately not folded in);
  making `ISseConnection.send` generic to accept named interfaces (§9).
...
Probed with `deno check` under the repository's own `deno.json` (`strict`, `exactOptionalPropertyTypes`),
not inferred. Each row is the reason a design decision in §3 reads the way it does.
...
- **Why:** an optional `peek?` returning `undefined` conflates two different answers — "this
  ...
  `onSignal?`, `routeUpgrade?`) are all platform-capability ports where absence is a real, reportable
  state; `peek` is a service method every implementation of a registry can answer. M51b's required
  `IGraphqlService.subscribe` is the matching precedent. The in-repo cost is nil: both stand-ins are
  built with `as unknown as` (§1), so nothing in the workspace needs a change to keep compiling.
...
- **Why:** a read-only projection would be a second type for the same thing and would have to be
  rebuilt per call; the milestone's property is that the *lookup* does not allocate a registry
...
- **Why:** the whole point is that the registry is not mutated. Touching `#neverJoined` has two wrong
  outcomes: it resurrects an abandoned room past its reclaim point when the peeked name exists, and
  it reclaims on a read path that promises to change nothing.
...
- **Why:** the WebSocket registry's `#neverJoined` design carries a latent trap — a caller holding a
  reference to a reclaimed entry publishes to a detached object while a new one serves the name — and
  importing it into SSE would convert the documented "hold a channel reference at startup" pattern
  into silent partial delivery, which is a worse defect than the growth it fixes. `peek` closes the
  read path X3-8 names, which is this milestone's scope; a safe reclamation design is named in §9.
...
`SsePluginOptions` are untouched, and `peek` needs no configuration. `deno.json` manifests, capability
tokens and plugin `provides` lists are all unchanged.
...
2. Widen `JsonValue`'s object arm to `unknown` — every `@ts-expect-error` in `type-contracts.test.ts`
   becomes an unused-directive compile error.
...
- **A required `peek` breaks an external replacement implementation of one of the two services.** Mitigated
  by the CHANGELOG entry stating the one-line addition, and bounded by the fact that both contracts
  have exactly one in-repo implementor and every stand-in is structurally cast (§1).
...
- **Reclaiming empty SSE channels.** §3.9 — a behaviour change that risks silent partial delivery for
  an application holding a channel reference. A safe design (reference-stable channels, or an
...
- **Making `ISseConnection.send` generic so named interfaces assign.** The interface/index-signature
  ...
  signature of a released method for an ergonomics gain the `type`-alias workaround already provides.
- **Constraining `WebSocketRoom.broadcastJson<T>` to `JsonValue`.** A second breaking change with its
  own consumers; the WebSocket path has no equivalent false documented guarantee driving it.
