...
§3.3 is two changes rather than the one the ROADMAP names.
...
| R13 the callback receives the FULL path                      | **measured** (real handler, fake fs)                                                                                   | `urlPrefix: '/assets'`, request `/assets/app-A9acsx54.js` → callback sees `/assets/app-A9acsx54.js`. Request `/assets` and `/assets/` → `/assets/index.html`. Root-mounted, `/` → `/index.html`.                                                                                             |
...
- **Decision:** two changes, landing together. (a) `parseHeaders` accepts the unquoted token form
  for both `name` and `filename` — value to the next `;` or line end, then trimmed, parameter name
  matched case-sensitively (R9). (b) A part whose resulting `name` is absent or empty is DROPPED
  rather than renamed.
- **Why:** (b) alone is a regression. R6 measures that an unquoted `name=x` currently arrives as
  ...
  and (b) together make our parser agree with the platform on all four measured cases. R8 is why the
  drop rule is "absent or empty" rather than "absent".
...
- **Why:** the ROADMAP offers keeping a sentinel "unrepresentable as a real field name" as an
  ...
  Matching the platform is also the stated reference (M94b), and the platform emits nothing.
...
| `CHANGELOG.md`                                                                           | The two multipart behaviour changes and the CSRF default, with migration text.                         |
...
| `packages/common/test/unit/form/multipart-platform-parity.test.ts` (new) | `form/multipart-parser.ts`                            | One table run through `parseMultipart(body, contentType)` AND `new Response(body,…).formData()`, asserting agreement on: nameless part beside a legitimate `unknown` field (R5), unquoted `name=x` (R6), unquoted `filename` (R7), empty `name=""` (R8), unquoted-token trimming and case sensitivity (R9). The platform is the oracle, so the assertions cannot encode our own bug.                                                                        |
...
- **§3.4 is a behaviour change to form parsing, which sits under authentication.** A part that used
  ...
  the oracle in every assertion, so the new behaviour is the web's; both consumers (R12) are driven
  directly; and CHANGELOG plus `docs/upgrading.md` carry the reader-side action rather than leaving
  it to discovery.
