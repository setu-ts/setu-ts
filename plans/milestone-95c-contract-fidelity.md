...
`string | Uint8Array | ArrayBuffer | Blob | URLSearchParams | object`, and the coercion becomes
explicit per shape: bytes pass through unchanged, a `Blob` is awaited to bytes, `URLSearchParams` is
serialised with its own `toString()`, and only a plain object reaches `JSON.stringify`. The
documented sentence at `application.ts:75` is rewritten to say which shapes are carried verbatim.
...
rather than JSON-stringified, so the class cannot reappear for the next value someone passes.
...
| `packages/kernel/test/unit/inject-body-shapes.test.ts` (new)             | `application/application.ts`                          | One table over `string`, `Uint8Array`, `ArrayBuffer`, `Blob`, `URLSearchParams` and a plain object: each asserts the exact bytes `await ctx.request.bytes()` yields, so the `{}`-for-everything outcome (M5) cannot return. Plus the named refusal for a shape outside the union, and that `URLSearchParams` defaults its content type the way the string path does.                                                                                                                                                                                                                                                                                                               |
