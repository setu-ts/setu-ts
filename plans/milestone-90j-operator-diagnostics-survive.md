...
> **Status:** Planning. Branch: `feat/m90j-operator-diagnostics-survive`. `main` is protected — all
...
one line: **7 catch-then-throw sites drop their cause**, spanning every shipped adapter family, and
of **162** `throw new` sites in the package **zero** pass a cause (re-measured on this tree:
`grep -rn "throw new" packages/database-plugin/src | wc -l` is 162 and the cause count is 0). Two of
the seven do not bind the caught value at all, so the information is unrecoverable rather than
merely unused. **X38-2:** `SerializedError` is exactly `{ name, message, stack?, cause? }` and
`readMember` is typed to those four keys, so pg's `code: '40001'`, `severity` and `constraint`,
MongoDB's `codeName` and the AWS SDK's `$metadata` are all dropped — log-based classification, such
as alerting on a `40001` rate, is impossible. **X28-7:**
`"Service Bus receiver error: AggregateError"` is the entire record, because an `AggregateError`
carries an empty `message` and its `errors` array is never read, while the publish path in the same
file logs a full cause chain.
...
- **In scope:** X38-2 (`SerializedError.classifiers?` from a documented scalar allowlist, and
  `errors?` for an `AggregateError`), X35-3 (the seven sites, plus the package-wide convention that
  keeps them fixed), X28-7 (the Service Bus receiver record, and the two sibling interpolation sites
  in the same file), and the doc deliverables C1–C3.
...
| the seven cause-dropping sites           | **re-measured on this tree** (the finding's line numbers are published `0.4.0`'s and two have drifted) | `drizzle-adapter.ts:279,366`; `prisma-adapter.ts:252`; `mongo-adapter.ts:175`; `cosmos-partition-key.ts:127`; `cosmos-adapter.ts:163`; `dynamo-client.ts:152`. **Three** — `drizzle:366`, `prisma:252`, `dynamo:152` — do not bind the caught value at all, so the information is unrecoverable rather than merely unused. The finding says two; it lists three. |
| the convention, re-measured on this tree | `grep -rn "throw new" packages/database-plugin/src \| wc -l` → **162**; the cause count → **0**        | Not one adapter's habit — the package has never passed a `cause`. This is why §3.3 ships a gate rather than seven edits.                                                                                                                                                                                                                                         |
| the exemplar site                        | `database-plugin/src/adapters/drizzle/drizzle-adapter.ts:364-369`                                      | `} catch { await outer.catch(() => {}); throw new Error('Drizzle transaction failed to start'); }` — no binding, no cause, and the message is the same eight words for a permission error, a dropped network or a bad search path.                                                                                                                               |
...
| C2 | The `database-plugin` sources contain 162 `throw new` sites and no stated convention, while `serialize-error.ts`'s own design assumes a cause chain exists to walk.                            | State the convention where an author will meet it, and enforce it, rather than fixing seven sites and trusting the next author to notice.                                   | A `CONTRIBUTING`-style note in `database-plugin/README.md`'s development section, plus the §3.3 gate that makes it checkable.       |
...
- **Decision:** the allowlist is `code`, `errno`, `syscall`, `severity`, `constraint`, `codeName`,
  ...
  anything else is dropped. `$metadata` is **not** included. Three explicit budgets bound the
  output: a per-value string cap (truncated with a marker rather than dropped, so a classifier stays
  recognisable), a cap on how many `AggregateError.errors` entries are serialized (carrying a count
  of those omitted), and the existing `MAX_CAUSE_DEPTH` on nesting.
- **Why:** X38-2's own argument is that a pg error carries the failing query and its parameters, so
  ...
  incomplete. But scalar-only bounds the SHAPE and not the SIZE, which is why the three budgets are
...
### 3.3 The seven sites are fixed, and a gate keeps the convention

- **Decision:** all seven become `catch (cause) { throw new Error('…', { cause }); }` (or the
...
- **Why:** 0 of 162 sites pass a cause, so this is a convention rather than seven mistakes, and
  seven edits leave the next author with nothing to notice. A source-scanning gate is the shape this
...
| `database-plugin/src/adapters/drizzle/drizzle-adapter.ts`                           | Sites `:279` and `:366` forward their cause.                                           |
| `database-plugin/src/adapters/prisma/prisma-adapter.ts`                             | Site `:252`.                                                                           |
| `database-plugin/src/adapters/mongo/mongo-adapter.ts`                               | Site `:175`.                                                                           |
...
| `database-plugin/test/unit/cause-chain-audit.test.ts` (new)                          | the package's own sources          | Scans `packages/database-plugin/src` for a `catch` that rethrows without forwarding the caught value; fails naming file and line; honours a `drops-cause: <reason>` marker. Verified to fail by reverting one site.                                                                                                                                                                                                                                                                                                 |
| `database-plugin/test/unit/{drizzle,prisma,mongo,cosmos}-adapter.test.ts` (extended) | six of the seven sites             | Each site's thrown error carries the injected driver failure as `cause`, and `serializeError` reports it at depth 1 **with its `code`** — these six wrap a driver response that carries one.                                                                                                                                                                                                                                                                                                                        |
| `database-plugin/test/unit/dynamo-client-seam.test.ts` (extended)                    | `adapters/dynamo/dynamo-client.ts` | The seventh site differs in kind and is asserted differently: `:152` catches a **`new URL()` failure**, a native `TypeError` carrying no driver classifier, so the assertion is that the native cause SURVIVES serialization. Requiring a `code` there would mean fabricating metadata the platform never produced.                                                                                                                                                                                                 |
...
1. Revert `drizzle-adapter.ts:366` to `} catch {` → the audit gate fails naming that line, **and**
   the real-backend case loses node-postgres's sentence. Both halves matter: the gate proves the
   convention is enforced, the backend case proves the enforcement buys the diagnostic.
...
- **A larger log record on the error path costs bytes at exactly the moment a system is under
  stress.** Mitigation: three budgets rather than one (§3.2) — seven scalar keys, a per-value string
...
- **Seven adapter edits touch code covered by fakes**, so a wrong `cause` argument can pass.
  Mitigation: each site gets a behavioural case asserting the cause reaches `serializeError`'s
  output, and the Drizzle one is additionally proven against a real pool.
