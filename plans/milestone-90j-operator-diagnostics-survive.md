...
| the seven cause-dropping sites           | `smoke/X35-FINDINGS.md` (X35-3), each named with a line                                            | `drizzle-adapter.ts:263,350`; `prisma-adapter.ts:252`; `mongo-adapter.ts:175`; `cosmos-partition-key.ts:127`; `cosmos-adapter.ts:163`; `dynamo-client.ts:152`. Two (`drizzle:350`, `prisma:252`, `dynamo:152`) do not bind the caught value.               |
...
| `database-plugin/src/adapters/drizzle/drizzle-adapter.ts`                           | Sites `:263` and `:350` forward their cause.                                           |
...
1. Revert `drizzle-adapter.ts:350` to `} catch {` → the audit gate fails naming that line, **and**
