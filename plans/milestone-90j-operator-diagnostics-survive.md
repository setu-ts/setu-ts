...
of **162** `throw new` sites in the package **zero** passed a cause when the finding was recorded.
The current tree has five wrappers that still drop their caught diagnostic; two transaction bridges
already rethrow the original rejection and are not part of this repair. **X38-2:** `SerializedError`
is exactly `{ name, message, stack?, cause? }` and `readMember` is typed to those four keys, so pg's
...
| `database-plugin/test/unit/{drizzle,mongo,cosmos}-adapter.test.ts` (extended) | four of the five sites             | Each site's thrown error carries the injected driver failure as `cause`, and `serializeError` reports it at depth 1 **with its `code`** where the driver provides one.                                                                                                                                                                                                                                                                                                                                              |
