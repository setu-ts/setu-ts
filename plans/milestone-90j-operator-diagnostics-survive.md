...
- **Decision:** the allowlist is `code`, `errno`, `syscall`, `severity`, `constraint`, `codeName`,
  ...
  anything else is dropped. `$metadata` is **not** included.
- **Why:** X38-2's own argument is that a pg error carries the failing query and its parameters, so
  ...
  incomplete. It also bounds the record's size, which matters because this output goes to a log
  transport on the error path. `$metadata` is an **object**, so carrying it means the spread the
  finding rejects or a second nested allowlist; and the AWS SDK sets `name` to the exception type
...
- **A larger log record on the error path costs bytes at exactly the moment a system is under
  stress.** Mitigation: the allowlist is seven scalar keys and the cause chain is already bounded by
  `MAX_CAUSE_DEPTH`; the `errors` array recurses under the same bound, which control 4 measures.
