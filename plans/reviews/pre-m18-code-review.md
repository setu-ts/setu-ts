...
| 2 | The kernel records env-var declarations as `envSpecs.push({ name: 'environment', spec })` (`application.ts:212`), hardcoding the name, so every validation failure reads "(declared by environment)" instead of naming the plugin that declared the variable. The `name` field exists precisely to attribute it.                                                                                                                                                                                                                               | Part 2 (`kernel`)                              |
...
| 6 | `common` JSDoc references `{@linkcode createRequestContext}` (`http.ts`, twice) — a kernel symbol a contracts package cannot resolve, so the link renders dead on JSR.                                                                                                                                                                                                                                                                                                                                                                         | Part 2 (or a doc tidy)                         |
