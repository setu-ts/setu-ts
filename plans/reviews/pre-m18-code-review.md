...
- `packages/testing` — pre-M18 scaffolding (M0, `e988568`), but it was the subject of M33 (merged as
  PR #78), whose own `review-milestone` pass covers it. Reviewing it here would have collided with
  that branch.
...
7. Report written to `.verify/review-p<N>-<slug>.md` (git-ignored scratch, never `git add`-ed) AND
   pasted into the part's PR body. The scratch file lives in the part's worktree, which is deleted
   once the part merges — the PR body is the durable copy, and without it the evidence for a merged
   part is unrecoverable.
...
fixed and 20 recorded below (of which 4 have since been closed, leaving 16 open). The recurring root
cause across Parts 3–6 was a **test double that did not honor the contract it stood in for** — no-op
response headers, a data source that ignored its query, an adapter named `ThrowingAdapter` that
never threw, a fixture whose two clocks returned the same number. Each one made a real defect look
correct.
...
| 3  | ~~**Ten** `metrics-plugin` test files import assertions from `https://deno.land/std@0.224.0/assert/mod.ts` — a hard-pinned remote URL that bypasses the workspace import map and pulls a second `std` into the lockfile. Part 1 fixed the one instance inside `common`; `metrics-plugin` is M19 (post-M18) so it sits outside all six parts.~~ **Fixed in the cleanup PR** — all 10 files converted to `expect` from `@std/expect` (506 assertions, none dropped) and the now-orphaned 33-entry `remote` section removed from `deno.lock`.                 | done (cleanup PR)                                     |
| 4  | ~~`@since` tags carrying milestone numbers as versions (`0.19.0`, `0.20.0`, `0.24.0`, `0.24.1`) — no such release exists; every package is `0.1.0`. Part 1 normalized the ones in `common`; 44 remain in `metrics-plugin`, `health-plugin`, and `telemetry-plugin`.~~ **Fixed in the cleanup PR** — normalized to `0.2.0` per the Part 1 precedent (44 in `src`, plus 12 more in test files the original sweep missed).                                                                                                                                    | done (cleanup PR)                                     |
...
| 16 | `QueueService.process()` silently REPLACES an existing processor for the same job name (`Map.set`), the same class as #14 in the CQRS bus. Both are undocumented; a fail-fast throw would match the kernel registry but changes behavior for deliberate re-registration, so the two should be decided together.                                                                                                                                                                                                                                            | maintainer decision (with #14)                        |
...
- ~~**Interaction with in-flight M33.** M33 lives on `feat/33-testing-package` and owns
  ...
  consumes, note it in the PR so M33 rebases deliberately.~~ **Retired** — M33 merged as PR #78
  after Part 6; it rebased onto the campaign's `common`/`kernel` fixes without conflict.
