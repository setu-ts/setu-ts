...
- **Decision:** Two templates. `rest` wires `RuntimePlugin`, `ConfigPlugin`, `LoggerPlugin`,
  ...
  `--template` keeps M34's minimal PLUGIN SET — `RuntimePlugin` alone — but not M34's file layout:
  §3.4's `honoe.config.ts` is emitted for every project, templated or not, because plugin-command
  discovery needs one seam that always exists. (An earlier draft of this decision said the default
  output was unchanged "byte-for-byte", which contradicted §3.4; §3.4 is the load-bearing side.)
...
| `test/unit/new-command.test.ts` (extended)        | `src/commands/new.ts`                           | Each template writes its plugin set into `honoe.config.ts`; `main.ts` imports `createApp`; no file mentions `-starter`; `microservice` + `cloudflare-workers` exits `2` with zero writes; unknown template exits `2`; omitting `--template` still emits `honoe.config.ts` but with `RuntimePlugin` as the only plugin. |
