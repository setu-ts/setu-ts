...
| ---------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `NatsOptions.streamSubjects` | `NatsBroker.connect` stream-creation branch | Supplied **and the stream absent** → `streams.add({ name, subjects })` verbatim, no `no_ack`. Supplied **and the stream exists** → connects, `streams.add` is never called and the existing configuration is untouched. Absent and the stream exists → the same. Absent and the stream absent → `JetStreamStreamError` naming the stream and both remedies (§3.2). |
...
| `test/integration/nats-real.test.ts` (new)               | `brokers/nats-broker.ts`        | Guarded on `NATS_URL`. Against a server where the stream is **absent**, with a concrete `streamSubjects` supplied through `MessagingPlugin` — without it §3.2 refuses before `streams.add`, so the case would assert the refusal rather than the creation: `app.start()` resolves and the stream exists afterwards. Against a pre-existing stream, with and without `streamSubjects`: `start()` resolves and no `add` is issued. One publish/subscribe round trip whose publish promise **resolves**.                                  |
