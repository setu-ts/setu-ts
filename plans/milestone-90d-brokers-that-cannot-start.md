...
- **NOT this milestone:** X28-5 and X28-6 — **M90b** (shipped; see the plan base note). X28-7 (the
  ...
  ungrouped in the ROADMAP register and not folded in here. Kafka RPC (`request`/`respond`) beyond
  observing that its documented `MessagingNotSupportedError` refusal is now _reachable_ — the
  refusal's own behaviour is M14d's and unchanged. Pub/Sub RPC, recorded in X28 as **unrun** rather
  than broken.
...
| the PubAck is already discarded             | `messaging-plugin/src/brokers/nats-broker.ts:368-382`                                       | `realJs` is typed `publish(subject, data, options?): void` and the call is **not awaited**; `publishWithHeaders` returns `Promise.resolve()`. So no publish acknowledgement is observed today — which is what makes §3.2's `no_ack` free.           |
...
### 3.2 NATS: keep `['>']` as the default and add `no_ack: true`, with `streamSubjects` as the scope escape

- **Decision:** `streams.add` sends `{ name, subjects, no_ack: true }` when the resolved subject
  list contains the catch-all `'>'`, and `{ name, subjects }` when it does not. Subjects resolve
  from a new `NatsOptions.streamSubjects?: readonly string[]`, defaulting to `['>']`.
- **Why:** the default must not move, because the topic **is** the subject verbatim
  (`nats-broker.ts:374,381,421`) — a default of `['<prefix>.>']` would silently stop capturing every
  topic an existing application publishes, which is a wire change dressed as a bug fix.
  `no_ack:
  true` costs nothing observable: the publish acknowledgement is already discarded today
  (`nats-broker.ts:368-382` does not await `realJs.publish`), so the field removes a reply the
  broker never read. And the option exists because X28-2 is right that `'>'` ingests every subject
  on the server including other applications' — an operator who cares can scope the stream without
  the framework guessing on their behalf. The conditional `no_ack` is what keeps a scoped stream
  from carrying a field it does not need.
- **Test home:** `test/unit/nats-broker.test.ts` (three cases: default sends `no_ack`, a scoped
  `streamSubjects` sends the subjects and omits `no_ack`, a scoped list that still contains `'>'`
  sends both) and `test/integration/nats-real.test.ts` (a real server accepts what is sent).
...
| ---------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NatsOptions.streamSubjects` | `NatsBroker.connect` stream-creation branch | Absent → `['>']` with `no_ack: true` (today's subject set, now accepted). Supplied and containing `'>'` → those subjects with `no_ack: true`. Supplied and not containing `'>'` → those subjects, `no_ack` omitted. |
...
| `src/brokers/nats-broker.ts`                 | `streamSubjects` resolution; conditional `no_ack`; the JetStream probe moved inside a `try`; both named errors thrown with `cause`.                      |
...
| `test/unit/nats-broker.test.ts` (extended)               | `brokers/nats-broker.ts`        | Default `streams.add` payload carries `no_ack: true` and `subjects: ['>']`; scoped `streamSubjects` omits `no_ack`; a scoped list containing `'>'` keeps it; a rejecting `jetstreamManager` throws `JetStreamUnavailableError` with `cause`; a rejecting `streams.add` throws `JetStreamStreamError`; a `streams.info` rejection that is not "stream not found" throws `JetStreamStreamError` rather than the bare platform error. |
...
| `test/integration/kafka-real.test.ts` (new)              | `brokers/kafka-broker.ts`       | Guarded on `KAFKA_BROKERS`. `createApplication` + `MessagingPlugin({ broker: 'kafka' })`; `app.start()` **resolves** (the X28-1 regression guard); one publish/subscribe round trip; `request()` rejects with the documented refusal, which is only now observable.                                                                                                                                                                |
...
- **`no_ack: true` could turn out to affect something the plan believes it does not.** Mitigation:
  `nats-real.test.ts` publishes and consumes against a real server on a freshly created stream, so a
  regression in delivery — not just in creation — is what the suite measures. The claim that nothing
  observable changes rests on `nats-broker.ts:368-382` discarding the PubAck today, which is cited
  and re-checkable.
