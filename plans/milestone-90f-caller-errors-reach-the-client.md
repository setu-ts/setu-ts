...
- **Decision:** add `database-plugin/src/errors/classify.ts` exporting an internal
  ...
  chain with guarded reads. `DatabaseService` calls it at **four** sites — inside `wrapDataSource`'s
  per-method wrappers, around the `beginTransaction()` **acquisition**, in `transaction`'s existing
  `catch`, and around `query`'s delegation — and throws a package-owned `SerializationConflictError`
  (`409`) or `DatabaseUnavailableError` (`503`) carrying the original as `cause`. A `null`
  classification rethrows the original untouched.
