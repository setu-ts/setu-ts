...
- **Decision:** `secrets-plugin` gains `src/errors.ts` with `ReadOnlySecretProviderError` (`501`),
  ...
  both public operations inherit the refusal from one throw site. `resilience-plugin`'s three
  existing classes brand `this` in their constructors: `BulkheadFullError` → `503`,
  `CircuitOpenError` → `503`, `TimeoutError` → `504`.
