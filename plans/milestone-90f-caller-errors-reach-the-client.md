...
| the acquisition is OUTSIDE the `try` | `database-plugin/src/services/database-service.ts:114-115`                                         | `const txn = await this._adapter.beginTransaction();` is line **114**; the `try` opens on **115**. So a connection-acquisition failure — exactly where X35-2's pool timeout is raised — never reaches that `catch`, and a classifier placed only there would leave the headline row unfixed. |
...
| -- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1 | The ROADMAP's M90f table gives X35-2 and X32-7 the answer "`503` + `Retry-After`", while `ErrorResponseInit` (`common/src/errors/error-responder.ts:146-158`) has no header channel, so a hint cannot carry `Retry-After`. | Ship the **statuses** and decline the header, with the reason recorded rather than the shortfall left implicit (§3.5). Note the ROADMAP's single "`503`" for X32-7 is itself imprecise and is corrected here too: `BulkheadFullError` and `CircuitOpenError` answer `503`, `TimeoutError` answers `504` (§3.6). No hinted answer carries `Retry-After`, whatever its status. | ROADMAP M90f table corrected to "`503`"; `PUBLIC_API.md` states that hinted backpressure answers carry no `Retry-After` and why, beside the rate limiter's, which does. |
...
- **Decision:** add `database-plugin/src/errors/classify.ts` exporting an internal
  ...
  chain with guarded reads, **bounded by `MAX_CAUSE_DEPTH` and a visited set**, and returns `null`
  for an error this milestone already classified.

  Both bounds are requirements rather than defensive extras. A `cause` chain can be **cyclic** — an
  error whose `cause` is itself, or a two-node loop — and a guarded read prevents a throw, never a
  non-terminating walk; the one `catch` whose job is to contain failures would then hang the
  request. And `wrapDataSource` wraps the transaction-scoped data source too
  (`database-service.ts:119`), so an error already turned into a `SerializationConflictError` inside
  a repository call reaches `transaction()`'s catch, where a second classification would wrap the
  wrapper: the caller's `cause` would then be the first `SerializationConflictError` rather than the
  driver error the contract promises. An already-classified error is passed through untouched.
  `DatabaseService` calls it at **four** sites — inside `wrapDataSource`'s per-method wrappers,
  around the `beginTransaction()` **acquisition**, in `transaction`'s existing `catch`, and around
  `query`'s delegation — and throws a package-owned `SerializationConflictError` (`409`) or
  `DatabaseUnavailableError` (`503`) carrying the original as `cause`. A `null` classification
...
- **Decision:** `classifyDriverError` reads a **code** first and falls back to a small set of exact
  ...
  | DynamoDB                     | `TransactionConflictException`                                                   | the SDK's network-error name (`TimeoutError` / `NetworkingError`) — reproducible; `ProvisionedThroughputExceededException` and `ThrottlingException` — **documented-unverified**, not raisable against DynamoDB Local | DynamoDB Local in CI (conflict + network); the two throughput signals unverified |
...
- **Decision:** `secrets-plugin` gains `src/errors.ts` with `ReadOnlySecretProviderError` (`501`),
  thrown by `EnvProvider.set` — **the provider's only write method**. `EnvProvider` declares no
  `rotate` at all (`env-provider.ts:36-88` is `connect`/`disconnect`/`isReady`/`isHealthy`/`get`/
  `set`); `SecretsService.rotate()` delegates to `provider.set()` (`secrets-service.ts:108-111`), so
  both public operations inherit the refusal from one site. `set` **rejects** with it — it returns
  `Promise.reject(...)` today (`env-provider.ts:85-88`) and keeps doing so; a synchronous `throw`
  from a method typed `Promise<void>` would change the direct-call contract and bypass a caller
  using `.catch()`, which is the M52b/M52c/M70j defect class this repository has now hit four times.
  `resilience-plugin`'s three existing classes brand `this` in their constructors:
  `BulkheadFullError` → `503`, `CircuitOpenError` → `503`, `TimeoutError` → `504`.
...
| `SerializationConflictError` (`database-plugin`) | class | Thrown by `DatabaseService` at the four interception sites (§3.1); read by an application's retry loop — the `instanceof` X38-1 says a caller needs.                                               |
| `DatabaseUnavailableError` (`database-plugin`)   | class | Same, for the backpressure classification.                                                                                                                                                         |
| `ReadOnlySecretProviderError` (`secrets-plugin`) | class | Thrown by `EnvProvider.set`, the provider's only write method; `SecretsService.rotate()` reaches it by delegating there. Read by an application's `catch` and by `errorHandler` through its brand. |
...
| `database-plugin/src/services/database-service.ts`             | The four interception sites (§3.1): the `wrapDataSource` method wrappers, the `beginTransaction()` acquisition, `transaction()`'s catch, and `query()`. |
| `secrets-plugin/src/errors.ts`                                 | `ReadOnlySecretProviderError`.                                                                                                                          |
| `secrets-plugin/src/providers/env-provider.ts`                 | `set` throws it — the provider's only write method; `SecretsService.rotate()` reaches it by delegating to `set` (§3.6).                                 |
...
| `testing/test/unit/mock-context.test.ts` (extended)                         | `mock-context.ts`                        | `MockRequest.json()` throws the same class with the same brand — the double matches the real producer.                                                                                                                                                                                                                                                      |
...
| `database-plugin/test/integration/real-mongo-adapter.test.ts` (extended)    | `errors/classify.ts`                     | A `TransientTransactionError` label classifies as `conflict`, **and** a client pointed at a closed port classifies as `unavailable` — otherwise that column is asserted only against synthetic errors, which proves the mapping and not that the driver emits the signal.                                                                                   |
| `database-plugin/test/integration/real-dynamo-adapter.test.ts` (extended)   | `errors/classify.ts`                     | `TransactionConflictException` classifies as `conflict`, and a client pointed at a closed port classifies as `unavailable`. **`ProvisionedThroughputExceededException` and `ThrottlingException` are not reproducible against DynamoDB Local**, so those two entries are marked documented-unverified in §3.2 rather than claimed — the M30b/M52 precedent. |
...
| `secrets-plugin/test/integration/readonly-status.test.ts` (new)             | `errors.ts`, `providers/env-provider.ts` | Through a real application with `errorHandler`: **both** `POST /rotate/X` and a direct `set` answer `501` with the class's own sentence, under `'default'` and `'rfc9457'` — the two entry points funnelling through one throw site is the claim under test.                                                                                                |
