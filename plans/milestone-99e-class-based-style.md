...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Template selection           | `packages/cli/src/templates/choice.ts:66-95`                                                                           | `resolveTemplateChoice(args)` reads `--template` only. It refuses `--di` (`:69-74`), refuses an unknown name through the registry `Map`, and returns `{ ok, template? }`. `new` (`commands/new.ts:449`) and `generate app` (`commands/app.ts:200`) both call it, so one change reaches both verbs.                                                                                                                                                                                                                                                                                                            |
...
| C4 | The `--di` refusal (`choice.ts:69-74`, `new.ts:148-153`) points only at `--template class-based`.                                                                                           | It should name the axis that now exists.                                                                                                                     | Refusal text: `Use --style class-based (with --template rest or microservice)`. Update the `PUBLIC_API.md:7736` row.                                                                                                                                                                                                        |
...
- **Decision:** `CLASS_BASED_TEMPLATE` becomes
  `{ name: 'class-based', description, deprecatedFor: '--template rest --style class-based', ...composeHost(REST_RECIPE, 'class-based') }`.
  The new optional `TemplateDefinition.deprecatedFor?: string` has three readers:
  - `new --help` and `generate app --help` append `(alias of <deprecatedFor>)`.
  - The interactive template prompt omits the entry.
  - `resolveTemplateChoice` returns a `notice` the command logs once.

  The output stays byte-identical, and nothing is refused or scheduled for removal.
...
- **Test home:** `test/unit/style-flag.test.ts`; `test/unit/flags.test.ts` (inventory ↔ help).
...
- **Test home:** `test/unit/seams/host-seams.test.ts`.
...
- **Decision:** `seam-probe.test.ts` gains a third arm, `microservice --style class-based`. It
  ...
  and `job` (all class-based, so all ingress), then boots once and asserts:
  ...
  `scaffold-runs-e2e.test.ts` gains `microservice --style class-based` in `HOSTS` (format, lint,
  install, type-check, boot) and a Workers arm for the same pair (type-check and format only, as for
  the other Workers hosts). `BOOTABLE` stays the four template names, since the style arm is a host
...
| ---------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| None added to `src/index.ts` | —    | New symbols are module-internal: `TemplateRecipe`, `composeHost`, `TemplateStyle`, and the `classBased`/`deprecatedFor` fields. `composeHost` is read by the three template modules. `classBased` and `deprecatedFor` are read by `resolveTemplateChoice`, help rendering and the prompt, as §3.2–§3.6 name. |
...
| `TemplateDefinition.deprecatedFor` | help renderers, `resolveNewChoices`, `resolveTemplateChoice` | Help annotation, prompt omission, and a one-line notice.                                                                    |
...
| `src/templates/registry.ts`       | Add `classBased?` and `deprecatedFor?` to `TemplateDefinition`, with JSDoc.                                                                                                                                                          |
...
| `test/unit/style-flag.test.ts` (new)                         | `templates/choice.ts`, `commands/new.ts`, `commands/app.ts`, `templates/broker.ts` | Each §3.3 row, accepted or refused, with exit code `2` and the exact message. The notice is logged once for `--template class-based`. `--di` text names `--style`. `--broker redis --queue redis` on `microservice --style class-based` rewrites both wirings and emits the compose file. The C5 text is on the `rest --style class-based --broker redis` refusal. |
| `test/unit/seams/host-seams.test.ts` (extend or new)         | `seams/registry.ts`                                                                | A class-based set containing cqrs and events excludes the command, query and events seams and includes ingress. The functional set is unchanged. `scanSeamSpecs` is unchanged in both modes.                                                                                                                                                                       |
...
| `test/e2e/scaffold-runs-e2e.test.ts` (extend)                | the same host under generated permissions                                          | Format, lint, install, check and boot for `microservice --style class-based`, and format, lint and check for its Workers arm.                                                                                                                                                                                                                                      |
