...
> **Status:** Planning. Branch: `feat/43-sse-plugin`. `main` is protected — all work (implementation +
...
entirely on the Milestone 42 streaming primitive ([`IResponse.stream()`](packages/common/src/http.ts:169)
and [`IRequestContext.signal`](packages/common/src/http.ts:221)). The plugin owns SSE wire-format
...
- **Decision:** A pure, internal function `encodeSseMessage(msg)` returns the exact wire string;
  ...
  after the last field's `\n`, i.e. the event ends with `\n\n`). Omitted fields emit **no** line.
  `data` serialization: a `string` is taken literally and split on `\n` into multiple `data:` lines;
  any non-string is `JSON.stringify`-ed. `undefined` data throws `TypeError`. Comment frames are
  `: <text>\n\n`.
...
- **Test home:** `sse-frame.test.ts` asserts each frame byte-for-byte, including absent `id`/`event`/
  `retry` lines and multi-line `data`.
...
- **Test home:** `sse-service.test.ts` asserts the heartbeat interval is created only when
  `heartbeatMs` is set and that `retryMs` emits the initial `retry:` frame; `sse-integration.test.ts`
  observes a heartbeat comment frame delivered over a real socket.

### 3.5 Named channels and broadcast — registry get-or-create, broadcast skips closed

- **Decision:** `SseService` owns a `ChannelRegistry` (a `Map<string, SseChannel>`).
  `channel(name)` get-or-creates a channel. A channel's `add(conn)` registers membership; `remove`
  and `publish` are the other members. `publish(msg)` iterates members and calls `conn.send(msg)`,
  skipping any connection whose `isOpen` is false (a slow/closed member never breaks a broadcast).
  The connection's `onClosed` callback (§3.3) removes it from every channel, so disconnect auto-prunes
  membership.
...
- **Why:** `send` is synchronous to match the ROADMAP `conn.send({...})` API, so full async
  ...
  §14.5 — no memory leaks). These are internal constants, **not** options, so there is no dead-option
  surface; the behavior is still tested directly.
...
- **Why:** The health indicator and shutdown hook are behaviors the §6 tests assert, so each needs a
  ...
  design-decision home") — and the indicator's reported shape must be pinned at plan time so the test
  asserts the documented `{ status, data: { connections } }` fields, not an improvised shape. The
  teardown is the shutdown counterpart to the per-connection abort cleanup (§3.3): abort handles a
  single client disconnect, `onClose` handles the whole hub going down.
