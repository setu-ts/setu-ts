...
| `--workspace`                                   | `new`                                      | Creates a monorepo root instead of a project: a `deno.json` whose `workspace` globs are `./apps/*` and `./libs/*`, plus a `setu.workspace.json`, with no member. Both globs are written once, so neither a service nor a library ever rewrites the root. Refuses `--template` and a non-Deno `--runtime` rather than ignoring them.                              |
...
| Path                  | What it is                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docker/Dockerfile`   | One parameterized image for every member: `docker build -f docker/Dockerfile --build-arg MEMBER=orders -t orders:dev .` The context is the workspace root. |
...
Both are `managed`, for the same reason the discovery modules are: a stack that names two of three
