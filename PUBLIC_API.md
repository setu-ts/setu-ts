...
`IAccessTokenRevocationStore` and pass that exact instance to `AuthPlugin({ jwt: { ... } })` and the
...
shared implementation. Custom `RefreshTokenStore` implementations must preserve `familyId` and make
`rotate(jti, successor)` and `revokeFamily(jti)` linearizable per family: family revocation must
durably block later rotations and include any earlier successor.
...
| `csrf.headerName`    | `string`                               | `'x-csrf-token'`           | Header accepted as an alternative token source for `fetch` posts; an explicit name still wins. Read first, so a client that sends it triggers no body parse. No longer required for `multipart/form-data`, whose FIELD is read too (M94b) — but a token arriving as a multipart FILE part is refused |
...
- **Both form encodings are read for the token (M94b)** through the request's shared `formData`
  accessor (or the same `parseFormBody` fallback), so a token can arrive in a multipart FIELD — but
  only ever as a STRING: a token submitted as a multipart FILE part is refused as absent, because a
  client chooses freely whether a part carries a `filename` and a non-string must never reach the
...
`{ health: true, configuration: true, queues: true, traces: false, authorization: false,
cache: false, events: false, scheduler: false, realtime: false, storage: false,
outboundHttp: false }`
(`configuration` is `true` since M98e and `queues` since M98f, both below) — and the connector
serves a first inspector operation, `GET /v1/health`. The client reads it through
...
Broker-specific `logger` options remain string sinks (`{ error: (message: string) => void }`). When
a broker catches a failure it renders the error name, message, safe classifier fields, bounded cause
chain, and bounded aggregate members into that string; no structured driver object is passed to the
...
`min(maxSize * 2 + framing, maxBodyBytes)`. It bounds the parse THIS middleware triggers — which,
now that forms are read through the shared memoized accessor (M94b), is not necessarily the
request's first parse — not the initial read: the body is read lazily by the first consumer (M87),
and `RuntimePlugin({ maxBodyBytes })` (M90a) is the bound that covers every reader, including a CSRF
middleware that parsed the same form earlier. `IRequest` still exposes no body stream, so no
middleware can decline to read; capping the read for every reader needs a streaming request body.

Since M94b the middleware parses through the request's `formData` accessor (or the same shared
`parseFormBody` when the request omits it), and a part carrying NO `filename` under the field name
is a plain form value, not an upload — the web standard's answer; see the CHANGELOG migration note.
...
- `IMailer.isHealthy?()` (OPTIONAL, added in `0.7.0`) reports the transport's REACHABILITY, distinct
  ...
  `undefined`. Every built-in provider that ships a probe answers concretely — `log` is always
  `true` (it touches no network) and `sendgrid` calls the scopes endpoint — so the `undefined` arm
  is reached by the two providers that DROP their probe when the injected dependency cannot support
  one: `ses` with a client exposing no `isHealthy`, and `smtp` with a transport exposing no
  `verify()`. The `mail` health indicator reads reachability through this same method rather than
  past it to the provider, so the capability and the indicator cannot disagree. It exists on
  `IMailer` so a HOLDER of the capability can ask — `notification-plugin`'s email channel does, and
...
app.register(OpenApiPlugin({
  ...
  // Document each operation's success status from what its handler declared
  // (`@HttpCode` / `@Redirect`) instead of assuming 200. On by default.
...
### Deriving The Success Status From The Handler

With no `schema.response`, an operation is documented as
`200: { description: 'Successful response' }` — which is wrong for every route that answers
something else, and the wrongness is load-bearing: `@setu-ts/sdk`'s generator types the success body
under that key, so a `201` route's generated client types its success as `200` and a caller reading
the created record gets nothing.

`deriveResponseStatus` (default `true`) reads the status the handler DECLARED. Every route
`@setu-ts/decorator-plugin`'s `@HttpCode(status)` or `@Redirect(url, status?)` produces brands its
handler with `RouteResponseMetadata` (`@setu-ts/common`), and the generator reads that brand off
`RouteInfo.definition.handler`. No plugin imports another; the `Symbol.for`-keyed brand in `common`
is the whole channel, exactly as `RouteSecurityMetadata` is for guards and `RouteValidationMetadata`
is for request shapes.
...
- **A DECLARED `schema.response` always wins**, tested `!== undefined` rather than by length: an
  empty map is a caller saying "no documented responses". A route carrying `@ApiResponse(...)` is
  therefore untouched by the derivation.
- **The HANDLER carries the brand, not a middleware.** A guard and a validator ARE middleware; a
  success status is a property of the handler, and a decorated route may carry no middleware at all.
- **Only the status is derived.** `@ResponseHeader` contributes nothing — an OpenAPI response-header
  entry needs a schema and a description the declaration does not carry, so deriving one would put
  an under-specified `headers` object into every document that used the decorator.
- **A `@Redirect` route is documented under its `3xx`.**
- **`deriveResponseStatus: false`** restores the assumed `200` for every branded route. Like
  `deriveRequestSchemas: false` it disables this derivation only; owner exclusion, the `operationId`
  format and schema deduplication are unconditional.

Like `deriveRequestSchemas` and unlike `deriveSecurity`, this is ON by default, because nothing has
to be configured for it. It is safe to default on because the brand is new surface: no handler
written before `@HttpCode` existed carries one, so an application that changes nothing gets a
byte-identical document. What DOES change the document is adopting the decorator — which is the
point of it, and which moves that operation's generated success type from `200` to the declared
status.
...
- **A type union is always spelled `anyOf`, on both majors and every supported zod v4.** Draft
  2020-12 permits two spellings, and zod switched between them inside the supported range: through
  4.4 a union of bare types emitted `anyOf: [{ "type": "string" }, { "type": "null" }]`, and from
  4.5 it collapses to `"type": ["string", "null"]`. The transformer normalizes the collapsed form
  back, so one schema produces one document whichever in-range zod the application installs, the v4
  path agrees with the v3 path, and the document stays convertible to OpenAPI 3.0 (where a `type`
...
setu new my-app --template class-based          # byte-identical alias of --template rest --style class-based
...
setu generate app orders --devtool                   # + main.dev.ts, dev/check tasks, a devtool port
setu devtool enable orders                           # same composition, on a member that exists
setu devtool enable                                  # standalone project: no member name
...
| ----------------------------------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--runtime deno\|node\|bun\|cloudflare-workers` | `new`, `generate`                           | On `new`, selects the entry shape and manifest and defaults to `deno`. On `generate`, passes `SchematicOptions.runtime` to the schematic (read by custom schematics) and otherwise detects the target from the existing project's manifest. An unknown value is a usage error (`2`) on both.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `--devtool`                                     | `new`, `generate app`                       | Opts the project (or member) into the local diagnostics connector: emits a `main.dev.ts` development entry the production graph never imports, adds `dev` and `check` tasks, and — for a workspace member — records a `devtoolPort`. Refused by name on a non-Deno runtime, on a starter-composed template (kernel diagnostics must be enabled at construction), and, for `--devtool-port`, when the port is already used. The generated entry reads `SETU_DEVTOOL_SESSION_ID` and `SETU_DEVTOOL_SESSION_KEY` from the environment and refuses to start without a valid pair; it never generates, writes or prints one. In a workspace the launcher also sets `SETU_DEVTOOL_MEMBER` to the member it is inspecting, and the generated `scripts/dev.ts` forwards all three to that member's child alone, blanking them for every sibling. **All three names are published CLI surface**: renaming one is a breaking change to every scaffolded project, and the root workspace `dev` task's `--allow-env` grant is scoped to exactly them. |
| `--devtool-port <n>`                            | `new`, `generate app`, `devtool enable`     | The IPv4 loopback port the connector binds. Range-checked like `--port`; refused when it equals any member's application or devtool port. Absent, it is allocated in a workspace and defaults to 4919 standalone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `--style functional\|class-based`               | `new`, `generate app`                       | The decorator-and-DI axis, separate from `--template`: `functional` (default) installs neither `DecoratorPlugin` nor `DiPlugin`; `class-based` installs both together. Accepted on the styleable templates `rest` and `microservice`; refused with no template to apply to, with an unknown value, and on `full-stack`, which composes through a starter and has no controller or ingress seam. `--template class-based` is a byte-identical alias of `--template rest --style class-based`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `--broker <name>`                               | `new` (standalone)                          | The project's own message broker: `memory` (default), `redis`, `rabbitmq`, `nats`, `kafka`, `pubsub`, `service-bus`. Rewrites the template's `MessagingPlugin` wiring to that arm, adds the arm's connection variable to the generated dotenv pair, and emits `docker/compose.yaml` starting the broker so the scaffold can complete `app.start()`. Refused — each with its own reason, never silently ignored — on Cloudflare Workers (the runtime swap removes both wirings), starter-composed templates (`full-stack`; a factory-rendered plugin list would drop the rewrite), templates registering no messaging (`rest` and no template; `microservice` registers messaging in every style), unknown names, and `--workspace`/`generate app` (a workspace's transport is chosen once with `--transport`).                                                                                                                                                                                                                            |
...
| `--dry-run`                                     | `new`, `generate`, `add`, `workspace ports` | Prints `would create <path>` per file and performs zero writes and zero directory creations. On `add`, prints `would update <manifest>` instead, since it edits files rather than creating them. `workspace ports` prints `would update <path>` for each managed file instead of writing the workspace manifest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
two coherent compositions on the `--style` axis: functional is the default; `--style class-based`
opts into both decorators and dependency injection. The axis is separate from `--template`, and
`--template class-based` is a byte-identical alias of `--template rest --style class-based`.
...
| Class-based | `setu new app --template rest --style class-based` | `DecoratorPlugin` + `DiPlugin`, plus decorated controllers, injectable services, and class module registration.  |

`--style` is accepted on the styleable templates `rest` and `microservice`, by `setu new` and
`setu generate app`; it is refused with no template to apply to, with an unknown value, and on
`full-stack`, which composes through a starter and has no controller or ingress seam.

The independent `--di` flag is no longer supported. It is refused with guidance to use
`--style class-based` (with `--template rest` or `microservice`), which always installs the
decorator and DI pair together, and `--template nest` is refused with a message naming `class-based`
as its new name. `--template
full-stack` therefore has no DI opt-in from the CLI; an application
composing the starter directly still has `FullStackStarterOptions.di`.
...
| `new --workspace --di`                              | The independent DI switch is retired. Exits `2` directing the caller to `--style class-based` (with `--template rest` or `microservice`).                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
A class-based project (the `class-based` alias, or `rest`/`microservice` with `--style class-based`)
additionally emits `src/services/greeting.service.ts` and `src/controllers/greeting.controller.ts`,
registered through the seam barrels that `setu.config.ts` passes to `DecoratorPlugin`; the
controller declares its dependency in the class-position `@Inject('greeting-service')` list. It
supports every runtime target. On Cloudflare Workers, the microservice template swaps its
socket-bound messaging and queue plugins for the platform implementation.
...
| `serializeError(value)`                                | function | Serializes any thrown value into a plain `SerializedError` (`{ name, message, stack?, cause?, classifiers?, errors?, omittedErrorCount? }`) with the `cause` chain followed to a bounded depth. `classifiers` only carries strings, finite numbers, and booleans from `code`, `errno`, `syscall`, `severity`, `constraint`, `codeName`, and `statusCode`; payload objects such as query data and `$metadata` are omitted. `errors` is present only for an `AggregateError`. Classifier strings cap at 512 Unicode code points, `AggregateError.errors` caps at eight direct members, and one call produces at most 64 error nodes; a truncated aggregate reports `omittedErrorCount`. A non-`Error` value yields `{ name: 'Error', message: String(value) }`. Pure — no runtime-specific APIs. Here so the logger plugin (metadata normalization), the kernel (fallback-500 logging), `exceptions`, `grpc-plugin`, and `notification-plugin` can all serialize without importing one another. |
| `causeMessage(value)`                                  | function | Extracts a caller-facing message from any thrown value: its `message` when it is an `Error`, `String(value)` otherwise. The one-line counterpart of `serializeError` — that shapes a record for a log sink, this supplies the sentence that goes inside another error's message. Pure. Here because `resolveRegistryEntry` and `@setu-ts/scheduler-plugin`'s declared-job refusal must produce the SAME sentence for the two arms of one option, and §2.2 forbids one reading the other's copy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `parseFormBody(body, contentType)`                     | function | Parses one request body as a form — the ONE parse all three `IRequest.formData?()` implementations (`@setu-ts/runtime`'s served HTTP path, the kernel's `inject()`, `@setu-ts/testing`'s `MockRequest`) share, and the function every accessor-less fallback path calls (M94b). Adopts the web `FormData` semantics — ordered `getAll` for repeated names, `entries()` in wire order, file-versus-text discriminated on `filename !== undefined` (`filename=""` is still a file) — over a framework-owned value whose bytes stay synchronous. Throws `UnsupportedFormEncodingError` (`415`) for a non-form content-type (JSON, a missing content-type, a multipart type with no `boundary=`); an UNPARSEABLE multipart body yields an EMPTY form, the promoted parser's released behaviour, documented as the accessor's limit (M94b)                                                                                                                                                         |
| `formEncodingOf(contentType)`                          | function | Classifies a content-type as `'urlencoded'` or `'multipart'` — case-insensitively and parameter-tolerantly — or `undefined` when the type is not a parseable form, a multipart type with no `boundary=` included. The ONE classifier the upload middleware's multipart guard and the CSRF verifier's form guard both read, replacing two private `includes()` checks that had drifted on case folding (M94b)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `FormBody` / `FormFile` / `FormValue` / `FormEncoding` | types    | The form read `IRequest.formData?()` resolves: a read-only `get`/`getAll`/`entries` view over `string \| FormFile` values, where `FormFile` carries the parser's own SYNCHRONOUS `Uint8Array` — the web `File`'s async, copying byte access is deliberately not adopted, because it would add a second full copy per file part on the upload path this abstraction exists to make cheaper. Named `FormBody`, never `FormData`, so it cannot shadow the global. The write side is not adopted: a request body is a fact about the request, not a collection a handler edits (M94b)                                                                                                                                                                                                                                                                                                                                                                                                             |
| `UnsupportedFormEncodingError`                         | class    | The not-a-form rejection behind `parseFormBody` and `IRequest.formData?()`, branded `415 Unsupported Media Type` at construction — the `MalformedRequestBodyError` precedent: answered in the application's configured format rather than a masked `500`, and exported so a handler catching its own `formData()` rejection can use `instanceof` (M94b)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| HTTP                | `IRequest`, `IResponse`, `IRequestContext`, `IMiddleware`, `MiddlewareFunction`, `NextFunction`, `RouteHandler`, `RouteDefinition`, `RouteSchema`, `SecurityRequirement`, `SECURITY_METADATA`, `RouteSecurityMetadata`, `withSecurityMetadata`, `securityMetadataOf`, `VALIDATION_METADATA`, `RouteValidationMetadata`, `withValidationMetadata`, `validationMetadataOf`, `RESPONSE_METADATA`, `RouteResponseMetadata`, `withResponseMetadata`, `responseMetadataOf`, `HandlerResult`, `ResponseSnapshot`, `ResponseSnapshotInit`, `UPGRADE_INTENT`, `WebSocketUpgradeIntent`, `setUpgradeIntent`, `upgradeIntentOf`, `isWebSocketUpgradeRequest`, `HTTP_STATUS_HINT`, `HttpStatusHint`, `withHttpStatusHint`, `httpStatusHintOf` |
...
- `withHttpStatusHint` brands an **`Error`** rather than a middleware function, and the reason it
  ...
  whose `status` is a plain `number` that an application authors (`FlagGuardOptions.statusCode` is
  the shipped example) and so may legitimately be any serveable status, not only an error one.
  `ErrorResponseInit.status` is guarded separately, by `resolveResponseStatus`, against the
  serveability bound `[200, 599]` rather than this error-ness one.
...
- `withResponseMetadata` brands a **`RouteHandler`** with a `RouteResponseMetadata`
  (`{ status: number }`), and `responseMetadataOf` reads it back — the same mechanism as the two
  brands above, applied to the status a handler answers with on success. Every route
  `@setu-ts/decorator-plugin`'s `@HttpCode` or `@Redirect` produces carries it, and
  `@setu-ts/openapi-plugin` reads it (`deriveResponseStatus`, on by default) to document the
  operation under that status instead of the assumed `200`. `RESPONSE_METADATA` is created with
  `Symbol.for` for the same cross-copy reason, and the brand is symbol-keyed and non-enumerable.

  The carrier is the HANDLER rather than a middleware function, and that is a design decision rather
  than a detail: a guard and a validator ARE middleware, while a success status is a property of the
  handler, and a decorated route may carry no middleware at all — branding a synthetic middleware
  purely to hold it would add a pipeline entry that does nothing. `status` must be an **integer in
  `200`–`599`**, the range the web `Response` constructor accepts; a brand outside it reads as
  ABSENT rather than being trusted, because a status no runtime could serve is not a status a
  document should claim. It carries the status ONLY: a response header is not represented, since an
  OpenAPI response-header entry needs a schema and a description a
  `@ResponseHeader('X-Total', '42')` declaration does not carry.
...
- `IRequest.formData?()` resolves a `FormBody` for both form encodings through the shared
  `parseFormBody`, memoized exactly like `json()` — the in-flight promise is cached, a rejection
  with it. It throws `UnsupportedFormEncodingError`, branded `415`, when the content-type is neither
  form encoding, and yields an EMPTY form for a multipart body it cannot parse: the promoted
  parser's released behaviour, stated as the accessor's limit rather than hidden behind a status.
  The same parse runs at all three producers, so an in-process `inject()` observes exactly what a
  served request observes; a request WITHOUT the optional accessor (an out-of-repo implementor)
  falls back to the same `parseFormBody(await request.bytes(), contentType)` — the fallback is the
  same function, not a second implementation. Optional on the `signal?`/`fs?`/ `raw?` precedent.
  Added in Milestone 94b.
...
| `ErrorHandlerOptions`   | type | Options for `errorHandler()` (`{ format?, includeStackTrace?, logErrors?, maskInternalErrors?, respond? }`); `respond(error: HttpError, ctx: IRequestContext)` returns or resolves a `HandlerResult` or `undefined` fallback. |
...
- **Application-owned caught-error response**: `ErrorHandlerOptions.respond` runs after a caught
  value has been normalized, status-hinted or masked, and resolved to a serveable status. It
  receives that `HttpError` and the live `IRequestContext`; returning or resolving a `HandlerResult`
  built through `ctx.response` bypasses framework formatting, so the application owns the returned
  status, headers, and body (for example, an HTML error page). Returning or resolving `undefined` is
  the only fallback signal and preserves the configured formatter exactly. Logging retains the
...
| Export                                                       | Kind     | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DecoratorPlugin`                                            | function | Plugin factory — registers `MetadataStore`, routes/services, explicitly activated `@Module` trees, and classes explicitly listed in `ingress` during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `Processor`                                                  | function | Method decorator — registers a bound handler with `IQueue.process(name, handler, options)` during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `Cron`/`Every`                                               | function | Method decorators — schedule the bound handler by its method name through `IScheduler.cron()` / `every()` during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `OnEvent`                                                    | function | Method decorator — subscribes the bound handler to an `IEventBus` event type during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `Subscribe`                                                  | function | Method decorator — subscribes the bound handler to an `IMessageBroker` topic during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `Gateway` with `OnOpen`/`OnMessage`/`OnClose`                | function | Class and method decorators — assemble one `IWebSocketService.route()` declaration from a gateway class during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `CommandHandler`/`QueryHandler`                              | function | Method decorators — register bound `ICommandHandler` / `IQueryHandler` objects during `onInit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `UseIngressBehaviors`/`UsePipelineBehaviors`                 | function | Method decorators — scope `IIngressBehavior` to one supported non-HTTP ingress handler, or `IPipelineBehavior` to one command/query handler; wrong decorator category is refused at startup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Export                       | Kind | Purpose                                                                                                                                                        |
| ---------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DecoratorPluginOptions`     | type | Options for `DecoratorPlugin()` (`autoDiscover?`, `controllersPath?`, `controllers?`, `services?`, `ingress?`, `modules?`, `enforceSchemas?`, `enforceRoles?`) |
...
- **Escaping is on for both default arms, and is the RUNTIME's doing rather than the plugin's.** A
  plain `(props) => string` component is a valid `Component<P>` and its output is returned
  unchanged, so a hand-written template literal escapes nothing — write views with JSX or the `html`
  tag. Both default arms escape interpolations through their rendering runtime; `raw()` is the
  documented opt-out. Rendered output is always a primitive string.
