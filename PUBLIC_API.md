...
| `--template full-stack` on a broker transport       | That template composes its curated plugin set through a starter factory, so a broker's `MessagingPlugin` rewrite cannot be applied without silently leaving the starter default in place. The command refuses it rather than scaffolding a member that reaches nobody. `grpc` is supported: its direct `GrpcPlugin()` contribution is registered after the starter factory returns and before the application starts. `http`, `grpc`, and `memory` are allowed; the root gains `nodeModulesDir` when the frontend member arrives. |
