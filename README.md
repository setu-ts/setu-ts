...
> [!IMPORTANT]
> **Status: all 47 packages are published in `v0.1.0-alpha.8` and are live on JSR.**
>
> <!-- version:history -->
>
...
```bash
# Deno
deno add jsr:@setu-ts/kernel@^0.1.0-alpha.8 jsr:@setu-ts/runtime@^0.1.0-alpha.8

# Node
npx jsr add @setu-ts/kernel@^0.1.0-alpha.8 @setu-ts/runtime@^0.1.0-alpha.8

# Bun
bunx jsr add @setu-ts/kernel@^0.1.0-alpha.8 @setu-ts/runtime@^0.1.0-alpha.8
```

**The `@^0.1.0-alpha.8` is required, not decorative.** JSR does not point `latest` at a prerelease,
so omitting the version fails outright:

```
error: jsr:@setu-ts/kernel has only pre-release versions available.
Try specifying a version: deno add jsr:@setu-ts/kernel@^0.1.0-alpha.8
```
...
All 47 workspace members are published on JSR, in `v0.1.0-alpha.8`: the core (`common`, `kernel`,
