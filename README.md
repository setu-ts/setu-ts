...
> [!IMPORTANT]
> **Status: all 47 packages are published in `v0.5.0` and are live on JSR.**
>
> <!-- version:history -->
>
...
```bash
# Deno
deno add jsr:@setu-ts/kernel@^0.5.0 jsr:@setu-ts/runtime@^0.5.0

# Node
npx jsr add @setu-ts/kernel@^0.5.0 @setu-ts/runtime@^0.5.0

# Bun
bunx jsr add @setu-ts/kernel@^0.5.0 @setu-ts/runtime@^0.5.0
```

**The `@^0.5.0` is recommended, not required.** `v0.5.0` is a normal release, so a bare
...
All 47 workspace members are published on JSR, in `v0.5.0`: the core (`common`, `kernel`, `runtime`,
...
| ---------------------------- | ------------------------------------------------------------------------------------------ |
| **Minor** — `0.4` → `0.5`    | Breaking changes. Every one is listed in [CHANGELOG.md](CHANGELOG.md) with migration text. |
| **Patch** — the third number | Fixes and additions only. Safe to take without reading anything.                           |

Under semver a `^0.5.0` range means `>=0.5.0 <0.6.0`, so a caret pin absorbs patches and stops at
