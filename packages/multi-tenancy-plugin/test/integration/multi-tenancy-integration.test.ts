...
describe('multi tenancy integration', () => {
  it('integration — full pipeline with header resolver', async () => {
    ...
    expect(middlewareNextCalled).toBeTruthy();
  ...
  it('integration — cross-tenant isolation via memory store', async () => {
  ...
  it('integration — required:true rejects unresolvable request', async () => {
    ...
    expect(shortCircuitStatus).toEqual(400);
    expect(!handlerNextCalled).toBeTruthy();
  });
});
