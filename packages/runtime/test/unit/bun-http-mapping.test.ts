/**
 * Tests for Bun HTTP request/response mapping.
 */
...
import {
  mapBunRequest,
  mapSnapshotToBunResponse,
} from '../../src/adapters/bun/bun-http-mapping.ts';

describe('Bun HTTP mapping', () => {
  describe('mapBunRequest', () => {
    it('maps GET request correctly', () => {
      ...
      const request = mapBunRequest(nativeRequest);
  ...
  describe('mapSnapshotToBunResponse', () => {
    it('maps snapshot with string body', () => {
      ...
      const response = mapSnapshotToBunResponse(snapshot);
