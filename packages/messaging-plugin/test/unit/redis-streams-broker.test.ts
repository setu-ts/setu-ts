...
Deno.test('RedisStreamsBroker - unsubscribe clears poll interval', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, {
...
Deno.test('RedisStreamsBroker - publish throws when not connected', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const broker = new RedisStreamsBroker(runtime, serializer, {});
...
Deno.test('RedisStreamsBroker - subscribe throws when not connected', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const broker = new RedisStreamsBroker(runtime, serializer, {});
...
Deno.test('RedisStreamsBroker - handler failure does NOT acknowledge message', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient({
...
Deno.test('RedisStreamsBroker - in-flight guard prevents overlapping polls', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient({
...
Deno.test('RedisStreamsBroker - logger is used for error logging', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient({
...
Deno.test('RedisStreamsBroker - disconnect clears all active subscriptions', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, {
...
Deno.test('RedisStreamsBroker - isReady returns false before connect', () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const broker = new RedisStreamsBroker(runtime, serializer, {});
...
Deno.test('RedisStreamsBroker - isReady returns true after connect', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, { client: fakeClient });
...
Deno.test('RedisStreamsBroker - isReady returns false after disconnect', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, { client: fakeClient });
...
Deno.test('RedisStreamsBroker - custom pollIntervalMs is used', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, {
...
Deno.test('RedisStreamsBroker - custom blockSizeMs is used', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, {
...
Deno.test('RedisStreamsBroker - multiple subscriptions to same topic work', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
  const broker = new RedisStreamsBroker(runtime, serializer, {
...
Deno.test('RedisStreamsBroker - validateClient rejects null', () => {
  expect(validateClient(null)).toBe(false);
});

Deno.test('RedisStreamsBroker - validateClient rejects non-object', () => {
  expect(validateClient('string')).toBe(false);
  expect(validateClient(123)).toBe(false);
  expect(validateClient(true)).toBe(false);
});

Deno.test('RedisStreamsBroker - validateClient rejects object with missing methods', () => {
...
Deno.test('RedisStreamsBroker - reject invalid client during connect', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
...
Deno.test('RedisStreamsBroker - subscribe throws on non-BUSYGROUP xgroup error', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  ...
  await expect(broker.subscribe('test.stream', async () => {})).rejects.toThrow('Some other error');

  await broker.disconnect();
});

Deno.test('RedisStreamsBroker - in-flight guard prevents concurrent polls', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
  const fakeClient = new FakeRedisStreamsClient();
...
Deno.test('RedisStreamsBroker - subscribe with entryId containing non-numeric timestamp handles gracefully', async () => {
  const runtime = createFakeRuntime();
  const serializer = new JsonSerializer();
...
Deno.test('RedisStreamsBroker - resolveClient calls loadIoredis when no client injected', async () => {
  // This test exercises the loadIoredis() fallback path (line 65)
  // by creating a broker without injecting a client.
  // Note: This requires ioredis to be available and may connect to a real Redis.
  // We use a fake URL that will fail to connect, but the loadIoredis path will be exercised.
  ...
  // The important thing is that loadIoredis() was invoked during resolveClient
});
