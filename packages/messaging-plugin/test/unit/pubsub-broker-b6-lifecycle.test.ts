/**
 * B6 lifecycle tests for GcpPubSubBroker.
 *
 ...
 * open actually run. They use recording/deferred fake transports that route
 * replies back through the inbox callback.
 *
 * Covers:
 *   A. Concurrent FIRST request() calls share single inbox init
 *   B. Partial open failure with compensation
 *   C. Teardown idempotency
 *   D. Pub/Sub ownership — transport open does NOT independently create
 */
...
describe('GcpPubSubBroker B6 lifecycle', () => {
  ...
  describe('D. Pub/Sub ownership — single admin create', () => {
    it('broker #openReplyInbox issues one createSubscription; transport open does not add another', async () => {
      let createSubCount = 0;
      let openCount = 0;
      ...
      const transport: IPubSubTransport = {
        ...
        open: async () => {
          openCount++;
          // The transport.open should NOT call createSubscription itself.
      ...
      // Trigger the inbox open through request()
      const req = broker.request<string, string>('topic', 'test', { timeoutMs: 10000 });

      // Wait for the create to complete
      await new Promise((r) => setTimeout(r, 10));

      // At this point: createSubscription called once by #openReplyInbox,
      // then open called once. If open also created, we'd see 2.
      expect(createSubCount).toBe(1);
      expect(openCount).toBe(1);
