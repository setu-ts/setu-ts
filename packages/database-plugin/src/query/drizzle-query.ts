 ...
 *
 * Supply the application's own instance type explicitly. The helper cannot
 * infer a schema through a capability token, and omitting `TInstance` infers
 * `unknown`.
 *
 * @typeParam TInstance - The type of the application's configured Drizzle instance
 ...
 * @example
 * ```typescript
 * const native = getDrizzle<typeof drizzleDb>(databaseService);
 * const rows = await native.select().from(users);
 *
 ...
 * @since 0.2.0
 */
export function getDrizzle<TInstance>(scope: IDatabaseService | IUnitOfWork): TInstance {
  return readDrizzleQueryHandle(scope) as TInstance;
}
