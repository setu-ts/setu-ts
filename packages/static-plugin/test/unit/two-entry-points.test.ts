...
describe('two entry points', () => {
  ...
  it('should use the same implementation under non-default config', async () => {
    const content = new TextEncoder().encode('hello world');
    const fs = {
      stat: (path: string) => {
        if (path.endsWith('test.txt')) {
    ...
    // The handler registered by the plugin should use the same options
    const handler = createStaticHandler({
      fs,
      root: '/root',
      urlPrefix: '/assets',
    ...
    const ctx = {
      ...
      request: {
        ...
        method: 'GET',
        headers: new Headers(),
        url: 'http://localhost/assets/test.txt',
        json: () => Promise.resolve({}),
        text: () => Promise.resolve(''),
        bytes: () => Promise.resolve(new Uint8Array()),
      },
    ...
    expect(ctx.response._status).toBe(200);
    expect(ctx.response._headers.get('Cache-Control')).toBe('no-store');
