...
**In Roo's Verify Milestone mode, do NOT stash — stop and report blocked instead**, naming the dirty
paths, so the pipeline can switch to Code mode and commit (see
`.roo/rules-verify-milestone/01-verify-only.md`). That mode cannot commit, so a stash it takes is a
stash nobody pops if the run ends early. The stash route above is for an agent that owns the tree.
