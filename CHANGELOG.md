...
- **`@setu-ts/view-plugin` (new package)** — server-rendered HTML as a capability. Registers an
  ...
  `@Render(Component)` in `@setu-ts/decorator-plugin` (type-checks the handler's return against the
  component's props; `CAPABILITIES.VIEW` joins its `optionalDependencies`; a rendered route with no
  provider fails at `register()` naming both remedies) and the free
