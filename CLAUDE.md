...
- **Milestone 63** (`packages/cli` — scaffold repairs) — complete (PR #153). Four defects found by
  ...
  **D1:** a project scaffolded on release day could not `deno install` at all — Deno 2.9 refuses a
  ...
  #329): the two doc sites, which its six-site list never named, and a SECOND occurrence in the CLI
  test — whose sibling comment three assertions away had been corrected, so the file looked done.
  The re-probe is recorded there: on Deno 2.9.6 a manifest declaring only `experimentalDecorators`
  still reports `TS7006` for an implicit `any`, so `strict` survives, while the control declaring
  `strict: false` beside it type-checks cleanly. Compiler options are now per template
...
- **Milestone 90d** (`packages/messaging-plugin` — the two brokers that cannot start) — complete (PR
  ...
  that the server refuses — and whose `no_ack` "remedy" makes every publish reject unobserved — is
  replaced by an explicit `streamSubjects` option with no default; `connect()` now throws
  `JetStreamStreamError` naming both remedies when the stream is absent without it, and
  `JetStreamUnavailableError` (with the platform error as `cause`) when JetStream is off, both
  exported. X28-4: `IPubSubTransport`/`IServiceBusTransport` delivered callbacks gain optional
  `messageId`/`timestamp` (source-compatible), the adapters read the platforms' own fields, and the
  brokers copy them onto `MessageMetadata` only when present. **The new real-backend suites (per
  §3.6 `ignore:`-guarded, started as docker-run steps in BOTH workflows, pinned by
  `test/apps-gate.test.ts`) then found three more defects no fake could see**, fixed on this branch:
  `subscribe()` created consumers through `js.consumers.add`, which nats 2.29 does not expose
  (creation belongs to `jsm.consumers.add`) — every real NATS subscribe threw a bare `TypeError`;
  the delivery cast read `info.timestamp`, which nats spells `timestampNanos` above the float
  safe-integer range — an Invalid Date on every real delivery, now converted through BigInt; and the
  shared Kafka default consumer group collapsed to EMPTY assignments the moment an application
  subscribed two topics (kafkajs: "Consumer group received unsubscribed topics"), so the default
  group is now derived per topic — a **breaking** group-naming change, CHANGELOG'd. The e2e Kafka
  case is the first exercise of Kafka RPC against a real broker. Both fakes were corrected to reject
  what their real dependencies reject (the fixture-correction deliverable) — which is what made the
  two negative controls (constants reverted; catch-all reverted) fail loudly. The messaging
  README/PUBLIC_API also gained the per-broker `messageId`/`timestamp` table (C4, verified against
  source), the JetStream prerequisites with the `-js` flag (C2), and the per-broker real-backend CI
  column (C3); the three other packages' README export tables drifted from their barrels and were
  regenerated by `docs:exports` in passing.
- **Milestone 90f** (`common` + `runtime` + `kernel` + `testing` + `database-plugin` +
  `secrets-plugin` + `resilience-plugin` — caller errors reach the client correctly). Five status
  decisions made once: a malformed JSON body answers `400` through
  `MalformedRequestBodyError`/`parseJsonBody` in `common`, which all three `IRequest.json()`
  producers delegate to (fixing the kernel's alone would have left every SERVED request at `500`
  while `inject` answered `400`); driver conflicts and outages are classified in one place
  (`classifyDriverError`, internal) at `DatabaseService`'s four interception sites and thrown as
  `SerializationConflictError` (`409`) / `DatabaseUnavailableError` (`503`) carrying the driver
  error as `cause`; a read-only secrets write rejects with the package's first error class,
  `ReadOnlySecretProviderError` (`501`); and the three resilience shedding errors brand `503`/
  `503`/`504`. `Retry-After` is declined with cause and its absence is asserted, so a later addition
  is a deliberate change. Wrapping beats branding for the driver rows because `withHttpStatusHint`
  throws `TypeError` on a frozen error — the error path must not become the fault — and a
  package-owned class is the `instanceof` a retry loop needs. **The probe-first discipline paid
  twice**: the first implementation threw the classification STRING instead of a wrapped error
  (caught by a live-PostgreSQL probe before any test existed), and the classifier's numeric-code arm
  initially vetoed MongoDB's `TransientTransactionError` label, which rides an error carrying the
  numeric `code: 112` — both fixed with the measurements recorded in source. Live-backend proofs,
  all run here: SQLSTATE `40001` through real Drizzle SERIALIZABLE transactions AND through Prisma's
  `P2034` mapping, pool exhaustion → `503` against real node-postgres, Mongo `WriteConflict` → `409`
  against a real replica set, DynamoDB Local's connect refusal → `503` (measured: the AWS SDK
  surfaces the bare net error `ECONNREFUSED`, which is why the classifier reads errno strings).
  **Documented-unverified**: DynamoDB's `TransactionConflictException` and Cosmos's `449` — neither
  was reproducible against DynamoDB Local or the local emulator (both commits resolved serially),
  and Cosmos's outage is a startup refusal that never reaches a client. Bigtable's gRPC `ABORTED` is
  proven through the REAL `DatabaseService` transaction path rather than the emulator, which cannot
...
- **The `v0.6.0` closeout** — covers **two** runs against that version: the regression run (5
  ...
  between `v0.5.0` and `v0.6.0` because the regression run had driven M91–M94c at probe level only.
  The two sets are folded together **by defect shape rather than by run**, which is why no `M96` was
  opened — X51-1/X51-2 are M95b's mechanism on other packages and X47-1/X50-1 are M95c's title on
  other packages, so a separate milestone would have put two of them on one shape. **M95d**
  (documentation that survives contact, the M90h precedent) is the letter that carried the gate work
  — the URL-scheme payload for `check-example-behaviour.ts` and the `@since`-vs-shipping-version
  check, which nothing did before it.
...
- **A new PR carries the `maintainer-review` label** — `gh pr create --label maintainer-review`, or
  ...
  **The failure is silent**: the same file sets `review_status: false`, so there is no "review
  ...
  `@coderabbitai review`. **`enabled` is not the gate**, and a previous revision of this bullet said
  it was: the file read `enabled: false` from 2026-09-16 to 2026-09-22 on the belief that the label
  re-enabled review, which instead meant NO PR was reviewed automatically — labelled ones included —
  since `labels` filters review and cannot switch it on. PR #328 was blamed on a missing label at
  the time; #344 and #353 then went unreviewed while labelled, which found the real cause.
  Suspending automatic review means `enabled: false` AND deleting the `labels` list, so nobody reads
  a dead gate as a working one. The repo file also overrides the CodeRabbit dashboard, so the web
  toggle changes nothing while the file sets the same key. The maintainer's gate on EXTERNAL
  contributions is untouched, and does not depend on this rule being obeyed: applying a label needs
  the Triage role or above on this repository, so from a fork `--label` fails with a 403 and nothing
  is applied — that is the gate working, not something to retry around. AI_GUIDELINES §16.7 is
  canonical.
