...
| **Rules**            | Generation reads the target project's manifest and module directory, and never boots it; schematics are pure functions returning files, so `--dry-run` writes nothing. A generate refuses to overwrite any existing file, except one a schematic declares `managed` (only `src/modules/index.ts` today). Plugin-command discovery is the one path that DOES boot the app — via `setu.config.ts`, with no port and guaranteed teardown |
