...
| **Responsibilities** | Provide `IMessageBroker`; in-memory, Redis Streams, RabbitMQ, NATS, Kafka, GCP Pub/Sub, Azure Service Bus, and custom-injected broker adapters; serializer interface (`ISerializer`); `EventsMessagingBridge`; request-reply via `RequestReplyCore` |
| **Dependencies**     | `common`, `kernel`, `runtime`                                                                                                                                                                                                                       |
| **Public API**       | `MessagingPlugin()`; `EventsMessagingBridge()`; `InMemoryBroker`; `RedisStreamsBroker`; `RabbitMqBroker`; `NatsBroker`; `KafkaBroker`; `GcpPubSubBroker`; `ServiceBusBroker`; `JsonSerializer`; `IMessageBroker` (re-exported from `common`)        |
...
| **Responsibilities** | Add/process jobs; retry strategies; recurring jobs; concurrency control; Memory/Redis/RabbitMQ/SQS adapters; AWS SNS fan-out publisher; cloud DLQ promotion                                                                                                                                                                                                        |
| **Dependencies**     | `common`, `kernel`, `runtime`                                                                                                                                                                                                                                                                                                                                      |
| **Public API**       | `QueuePlugin()`; `MemoryQueue`; `RedisQueue`; `RabbitMqQueue`; `SqsQueue`; `SnsPublisher`; `IQueue` (re-exported from `common`)                                                                                                                                                                                                                                    |
...
| **Rules**            | Redis client is optional (injected or lazy-loaded via `npm:` specifier); Memory queue for testing; RabbitMQ adapter (polling via basicGet, TTL+DLX for delays); SQS adapter via AWS SDK `@aws-sdk/client-sqs` (injected or lazy-loaded); SNS publisher via AWS SDK `@aws-sdk/client-sns` (injected or lazy-loaded); DLQ forwards original body then deletes source |
...
| **Rules**            | Zero npm deps — JWT (HS256/RS256) and PBKDF2-SHA256 via Web Crypto (`runtime.subtle`/`runtime.randomBytes`); no JWT library. `jwt` and `authentication` always register; `authorization` registers only when `rbac` is supplied. `ctx.request.user` is the writable principal field; guards short-circuit (no `next()`) on 401/403. Refresh tokens (`RefreshTokenService`) + rate limiting (`rateLimitMiddleware`) shipped in M16b as standalone additions — no new tokens |
...
of a single capability token at startup. It is likewise a second provider of
`CAPABILITIES.MESSAGING` (M59, over Cloudflare Queues plus a Durable Object reply inbox) — an
application registers `MessagingPlugin` **or** the Cloudflare `messaging` arm, never both.
...
| **Responsibilities** | Project scaffolding; monorepo workspaces; code generation; plugin-aware generators                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Public API**       | The `setu` command (`new`, `new --workspace`, `generate`, `generate app`, `commands`); `runCli`, `deriveNames`, `detectPlugins`, `PROGRAM_NAME`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| **Rules**            | Generation reads the target project's manifest and its generated-artifact directories, and never boots it; schematics are pure functions returning files, so `--dry-run` writes nothing. A generate refuses to overwrite any existing file, except the CLI-owned seam barrels a schematic declares `managed` (one `index.ts` per generated family). It also refuses a name that would collide with an existing artifact's HTTP path or injection token. Generation has two coherent styles, selected by the template and persisted in the generated manifest: the default is FUNCTIONAL — no `DecoratorPlugin`, no `DiPlugin`, `ctx`-first handlers and plain-function services — while `--template class-based` installs both plugins together and switches every style-sensitive schematic to decorated classes. There is no independent `--di` axis; a project's installed package set IS the selector, so a later `generate` cannot silently revert to the other style. Every host — including the no-template one — wires the seams whose plugins it registers, so `setu generate route` is a first-class decorator-free path. Plugin-command discovery is the one path that DOES boot the app — via `setu.config.ts`, with no port and guaranteed teardown. A **workspace** is a Deno workspace by default, or an npm one under `--runtime node|bun` (Cloudflare Workers is refused — each Worker is its own deploy unit), whose root declares members by GLOB (`"workspace": ["./apps/*"]`, or `"workspaces"` on the npm arm), so `setu generate app` creates a member directory and rewrites no manifest; framework pins live in each member's own manifest, because plugin detection reads one directory and never walks up. Cross-member service discovery is a CLI-owned `src/discovery/services.ts` per member, rendered from `setu.workspace.json` — the LOCAL development topology only; Compose and Kubernetes objects per member belong to M39. A template that a runtime serves through a DIFFERENT plugin declares a `RuntimeSwap` rather than refusing the pairing: `microservice` on `cloudflare-workers` swaps `MessagingPlugin`/`QueuePlugin` for `CloudflarePlugin`, which serves both capabilities from Cloudflare Queues, and contributes the module-level `queue` export, the Durable Object class the platform requires the application to own, and the wrangler stanzas it needs. A standalone `setu new` may select its OWN message broker and job queue with `--broker`/`--queue`, derived from the SAME transport registry the workspace `--transport` flag reads; each flag is REFUSED, never silently ignored, wherever it would be a no-op (Cloudflare Workers, starter-composed templates, templates registering no matching wiring), and the selected arm's Compose service ships with the scaffold so it can boot. Prompting is an OPTIONAL `ask` seam on `CliDependencies` that fails closed in three layers — absent by default, TTY-gated in the executable entry point, refused by Deno on a non-terminal — so every gate and script is non-interactive by construction |
...
| Aspect               | Detail                                                                                                                                  |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Purpose**          | Cloudflare Workers platform bindings: KV, R2, D1, Queues, Messaging, Cron, Cache API, Durable Objects                                 |
...
For an application-owned response to a caught error, `respond` can return or resolve a result built
with the live context's response writer (for example,
`ctx.response.status(error.statusCode).html(page)`). It runs after normalization, masking, and
served-status resolution; its returned result owns the response, while `undefined` falls through to
the configured formatter. Logging retains the unmasked diagnostic and records the selected
response's status. This is an explicit application branch, not content negotiation, and it does not
intercept the responder terminals below.
...
- **Refresh Token** — Issue new access tokens via the app-instantiated `RefreshTokenService`
  (**M16b**, shipped) — a service with rotation + revocation over a pluggable store, not an
  `IAuthStrategy` (a refresh token arrives in the request body, not as a passive header).
...
> The **WebSocket Plugin** shipped in Milestone 46 and is no longer a future extension point. The
> plugin does **not** hang off the kernel: it depends on `common` and the `http-adapter` capability,
> because an RFC 6455 handshake needs the native `Request` and answers with a 101 carrying a socket
> — neither of which the kernel's `IRequest`/`IResponse` abstraction can express, and the handshake
> itself stays an HTTP-adapter concern reached through the optional `IHttpAdapter.setUpgradeRouter`
> seam. **Deciding** the upgrade does not, though, and has not since M70a: the kernel's terminal
> handler resolves `IWebSocketService` from the registry and calls `routeUpgrade` after the §10
...
> through `IRequest`/`IResponse` — the former exposes no raw streaming body and the latter no
> trailing headers. RPC is dispatched from the kernel's terminal handler rather than intercepted in
> the adapter: the kernel resolves `IGrpcService`, asks `claims()` whether the path belongs to gRPC,
> and calls `handleRequest` with a `Request` rebuilt from the already-mapped body — a path gRPC does
> not claim falls through to route matching untouched. `IHttpAdapter.setRpcHandler` is deprecated
> and consulted by nothing. Only the **server** shipped: gRPC/Connect clients are generated by the
> consuming application's own toolchain, and the browser client story belongs to `@setu-ts/sdk`
> (Milestone 35).
...
> point: a GraphQL-over-HTTP exchange is a `POST` of `application/json` answered with JSON, so every
> byte of it fits `IRequest.json()` and `IResponse.send()` as committed. The plugin registers two
