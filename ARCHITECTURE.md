...
> does not hang off the kernel either: it depends on `common` and the `http-adapter` capability. A
> gRPC exchange cannot travel through `IRequest`/`IResponse` — the former exposes no raw streaming
> body and the latter no trailing headers — and the shared body mapping reads the request body
> before the pipeline runs. RPC is therefore intercepted inside the HTTP adapter's `fetch` path,
