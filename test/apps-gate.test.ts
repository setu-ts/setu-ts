...
describe('real-backend CI wiring', () => {
  ...
  it('declares the Mongo service, URI, and scoped database-plugin permission', async () => {
    ...
    // (guarded on POSTGRES_URL) and M81 added `127.0.0.1:8082` for the guarded
    // Cosmos emulator suite — endpoint-scoped in the manifest, because a CLI
    // `--allow-net` would REPLACE this block rather than union with it.
