...
| 11 | `database` (token)   | database-plugin           | `packages/database-plugin/src/plugin/database-plugin.ts:121`                    | `configuration-literal` | `service.isHealthy()` resolves to `adapter.isReady()` — lifecycle, not a reachability probe. **Recorded in `smoke/DEFECTS.md` (H-70c-3)** — out of scope.                                                                             |
